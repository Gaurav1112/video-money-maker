...
async function main() {
  ...
  console.log(
    `✅ ${storyboard.scenes.length} scenes, ${storyboard.durationInFrames} frames (${(storyboard.durationInFrames / 30).toFixed(0)}s)`
  );
  ...
  storyboard.scenes.forEach((s, i) => {
    console.log(
      `  Scene ${i + 1}: [${s.type}] frames ${s.startFrame}-${s.endFrame} (${((s.endFrame - s.startFrame) / 30).toFixed(1)}s)`
    );
  });
}
main();
