...
const NARRATIONS: Record<string, Record<number, SceneNarrations>> = {
  'load-balancing': {
    ...
    2: {
      ...
      closing:
        'Round robin simple lagta hai lekin workload asymmetry iska weakness hai. Stateless, uniform-cost services ke liye use karo — warna Least Connections better choice hai. Guru-sishya.in pe abhi practice karo.',
    },
    3: {
    ...
    4: {
      ...
      closing:
        'Consistent hashing distributed systems ka core primitive hai — load balancer se le ke sharding tak. Virtual nodes = uniform distribution. Guru-sishya.in pe full ring implementation dekhna mat bhoolna.',
    },
    5: {
    ...
    6: {
      ...
      gotcha:
        'WebSocket routing trap: L4 ka WebSocket ke saath problem hai — HTTP → WebSocket upgrade header ko decode nahi kar sakta, toh correct backend pe route nahi hoga. L7 zaruri hai WebSocket ke liye. Lekin L7 HTTP/1.1 keep-alive mein back-to-back requests same backend pe jaati hain — effective load distribution lose hoti hai. Solution: connection draining + HTTP/2 multiplexing ya proper timeout configuration.',
      closing:
        'L4 for speed, L7 for intelligence. FAANG systems dono layers simultaneously use karte hain — L4 at edge for raw throughput, L7 internally for microservices routing. Guru-sishya.in pe architecture diagrams dekho.',
    },
    7: {
    ...
    8: {
      ...
      gotcha:
        'DNS TTL ka failover trap: TTL 300 seconds aur datacenter suddenly down — 5 minutes tak users 5xx dekhenge. Lekin TTL 30 seconds karo — DNS resolver overload, costs shoot up, amplification attack surface badh jaata hai. Production solution: AWS Route 53 health-check integration ke saath TTL dynamically reduce karo — healthy state mein 300s, health-check fail pe 30s. Yeh latency vs availability tradeoff FAANG mein explicitly poochha jaata hai.',
      closing:
        'GSLB multi-region architecture ka glue hai. GeoDNS + health-aware TTL adjustment = resilient global system. Single-region se multi-region evolution ka answer isi pe hota hai. Guru-sishya.in pe architecture practice karo.',
    },
    9: {
      ...
      gotcha:
        'Service mesh vs load balancer confusion: "Netflix service mesh use karta hai — toh LB ki zaroorat nahi?" Wrong. Edge load balancer (Zuul) external internet traffic handle karta hai, service mesh internal microservice communication. Two separate layers — dono zaroori hain. Client-side load balancing fail tab hota hai jab stale Eureka cache hoti hai — Netflix solution: short TTL + heartbeat aur fallback to last-known-good list.',
      closing:
        'Real FAANG systems hybrid approach: edge LB + internal service mesh + client-side load balancing. Koi ek magic solution nahi hai. Case studies sikhte waqt yeh layering samajhna zaroori hai. Guru-sishya.in pe complete diagrams hain.',
    },
    10: {
      ...
      gotcha:
        'Most common interview mistake: implementation pe directly jump karna bina requirements clear kiye. Interviewer underspecified question deta hai: "Design a load balancer." Candidates immediately round robin explain karne lagte hain. Senior engineers pehle clarify karte hain — expected RPS, stateful ya stateless, compliance requirements, SLA targets. Yeh clarifications tumhe mid-level se senior-level dikhate hain — hiring bar clear karta hai.',
      closing:
        'Load balancing cheat sheet committed karo: RR, LC, CH, L4, L7, GSLB — each with one-line use case. FAANG mein concept depth + tradeoff clarity = hire decision. Guru-sishya.in pe mock interviews aur all 10 sessions complete karo.',
    },
  },
  ...
  caching: {
    ...
    4: {
      ...
      gotcha:
        'Redis single-thread bottleneck: KEYS * command production pe mat use karna — O(N) hai aur poora Redis block karta hai. Alternative: SCAN command iterates in chunks — non-blocking. Big key problem: ek Redis key mein 10MB data — serialization aur network transfer slow hoga. Solution: key ko smaller chunks mein split karo. LRANGE kisi bhi size list pe — O(S+N) hai, large lists pe slow. Yeh operational gotchas FAANG SRE interviews mein aksar poochhe jaate hain.',
      closing:
        'Redis sirf cache nahi — session store, queue, leaderboard, pub-sub, distributed lock sab kuch hai. Right data structure choose karna performance 10x impact karta hai. Guru-sishya.in pe Redis data types ke practice problems karo.',
    },
    5: {
      ...
      gotcha:
        'Memcached multi-threading advantage ka hidden cost: cache stampede mein multiple threads simultaneously same key ko DB se fetch karte hain. Redis single-threaded model mein yeh problem kam hai kyunki ek hi thread processes sequentially. Memcached mein explicit application-level locking zaroori hai stampede prevent karne ke liye. Interview mein: "Kyun Twitter ne Memcached se Redis migrate kiya?" — answer: rich data structures (sorted sets for timeline ranking) aur atomic operations.',
      closing:
        'Simple distributed cache = Memcached. Rich features, persistence, advanced data structures = Redis. New projects almost always Redis choose karte hain. Guru-sishya.in pe benchmark data aur decision framework dekho.',
    },
    6: {
      ...
      gotcha:
        'Distributed cache invalidation race condition: user ne profile update kiya → DB updated → cache invalidation event publish hua → meanwhile another request ne stale data cache mein load kar liya (cache miss pe DB se) → invalidation event process hua lekin cache already refreshed with old data? No — event ordering ensures: invalidate first, then the next cache miss will fetch fresh. Lekin event queue delay mein stale data serve ho sakta hai. Solution: strong consistency ke liye write-through + versioned keys.',
      closing:
        'Cache invalidation hard hai kyunki distributed systems mein no global clock aur no atomic operations across DB + cache. TTL + event-driven hybrid approach production mein best practice hai. Guru-sishya.in pe stampede patterns practice karo.',
    },
    7: {
      ...
      gotcha:
        'CDN cache poisoning attack: attacker manipulate karta hai cached response — ek malicious response cache mein store ho jaati hai, all users ko serve hoti hai. Prevention: strict Cache-Control headers, vary headers correctly set karo, cache key include karo Host header. Dynamic content caching trap: personalized content (user-specific) CDN pe cache nahi karni chahiye — vary: Cookie header use karo to exclude personalized responses from CDN cache.',
      closing:
        'CDN sirf static assets ke liye nahi — API responses bhi cache karo (public, non-user-specific). Edge computing CDN ko compute platform bana deta hai. Cache-Control headers master karo. Guru-sishya.in pe CDN configuration examples dekho.',
    },
    8: {
      ...
      gotcha:
        'Redis Cluster ka cross-slot transaction limitation: MULTI/EXEC transactions sirf single slot ke keys pe work karti hain — multi-key operations fail hoti hain agar keys alag slots pe hain. Solution: hash tags — {user}.session aur {user}.profile same slot pe jaate hain because hash tag {user} same hash. Yeh Redis Cluster mein advanced pattern hai jo most candidates miss karte hain. FAANG mein distributed transaction mein yahi trap aata hai.',
      closing:
        'Distributed caching = consistent hashing + replication + partition handling. Redis Cluster production ke liye, Sentinel simpler failover ke liye. Hash tags for co-located keys. Guru-sishya.in pe Redis Cluster architecture explore karo.',
    },
    9: {
      ...
      gotcha:
        'Fan-out on write vs read trade-off: celebrity problem. Priyanka Chopra ke 80 million followers — ek tweet pe 80M cache writes, toh fan-out on write infeasible. Solution: hybrid — normal users fan-out on write, celebrities (>threshold followers) fan-out on read + pre-computed partial timelines. FAANG interview mein yeh "hot key" problem explicitly poochha jaata hai — ek user ki activity disproportionate load kaisi handle karoge?',
      closing:
        'Real caching at scale = multiple strategies simultaneously. Fan-out on write, fan-out on read, hot key isolation — sab ka combination. Guru-sishya.in pe all three case studies detailed mein hain.',
    },
    10: {
  ...
  'database-design': {
    ...
    2: {
      ...
      gotcha:
        'N+1 query problem: ORM se 100 users fetch karo, phir har user ke liye separate query for orders — 101 queries instead of 1 JOIN. Solution: eager loading (JOIN), batch fetching, or dataloader pattern. Write amplification: har INSERT/UPDATE/DELETE ke liye every index update hota hai. Rule: indexing improves read, hurts write. Index too many columns → write performance degrades. EXPLAIN ANALYZE in PostgreSQL — always check query plan before adding index.',
      closing:
        'Indexing rule: add index where query selectivity is high aur access frequency justifies write overhead. B-tree for range queries, covering index for read performance, composite index with right column order. Guru-sishya.in pe query optimization labs hain.',
    },
    3: {
    ...
    5: {
      ...
      gotcha:
        'Denormalization trap: user ka city data 50 tables mein embed kiya — user ek city change karta hai, 50 rows update karni padengi. Solution: reference karo, embed mat karo frequently-changing data. But: user address at time of order — this SHOULD be embedded/snapshotted because it must not change retroactively. FAANG interview: "Design an e-commerce order schema" — order mein shipping address embed karo (snapshot at order time), but product reference karo (catalog link).',
      closing:
        'Schema design = access pattern first, normalization second. Embed for co-located reads, reference for independent updates, snapshot for historical accuracy. Real FAANG systems use both strategies contextually. Guru-sishya.in pe schema design problems hain.',
    },
    6: {
      ...
      gotcha:
        'MVCC snapshot isolation vs serializable trap: snapshot isolation prevents most read phenomena but NOT write skew. Write skew: two concurrent transactions read same data, make decisions based on it, write conflicting updates. Example: doctor on-call system — two doctors simultaneously check "is anyone else on call?" both see yes, both mark themselves "off-call" — nobody on call. Solution: SELECT FOR UPDATE (pessimistic locking) or SERIALIZABLE isolation. PostgreSQL SSI (Serializable Snapshot Isolation) detects write skew without full locking.',
      closing:
        'Isolation level choose karo based on correctness requirements: READ COMMITTED for most web apps, SERIALIZABLE for financial transactions. MVCC for read concurrency, explicit locks for write skew prevention. Guru-sishya.in pe transaction design problems practice karo.',
    },
    7: {
    ...
    8: {
      ...
      gotcha:
        'Connection pool timeout cascade: pool exhausted → new requests timeout → error rate spikes → alerts fire → engineers investigate → meanwhile pool stays exhausted because connections held by slow queries. Solution: statement timeout at DB level (kill slow queries), connection timeout at pool level (fail fast), circuit breaker to stop sending requests when DB struggling. FAANG SRE interview: "Your DB connection pool is exhausted during traffic spike — what do you do?" Step-by-step answer kaafi important hai.',
      closing:
        'Right pool size + statement timeouts + N+1 elimination = healthy database performance. PgBouncer for PostgreSQL, connection limits on application side. Monitor pool wait time, not just pool size. Guru-sishya.in pe performance debugging labs hain.',
    },
    9: {
      ...
      gotcha:
        'Database migration trap: zero-downtime migration. ALTER TABLE on 100GB table — hours of lock, zero writes. Solution: pt-online-schema-change (Percona) ya gh-ost (GitHub) — shadow table banaao, gradually copy, atomic swap. Expand-contract pattern: add column → backfill → use new column → remove old column — never in single deployment. FAANG: "How do you add a NOT NULL column to a 1-billion-row production table without downtime?" — yeh answer clearly know karna chahiye.',
      closing:
        'Real-world database evolution = incremental migration, expand-contract pattern, CDC for live migration, polyglot persistence. No big-bang rewrites in production. Guru-sishya.in pe all three case studies detailed walkthrough ke saath hain.',
    },
    10: {
      ...
      gotcha:
        'Common interview mistake: choosing a database before understanding access patterns. "I\'ll use MongoDB" without knowing query patterns — this is premature optimization. Interviewer sees this as junior behavior. Senior answer: "Let me understand what questions we need to answer. If we need flexible schema with complex queries → MongoDB. If write throughput is the constraint → Cassandra. If we need ACID transactions → PostgreSQL." Requirements first, technology second.',
      closing:
        'Database design cheat sheet committed karo: SQL vs NoSQL decision matrix, indexing rules, sharding strategies, replication patterns. FAANG mein yeh sab explain karna with tradeoffs = senior-level signal. Guru-sishya.in pe all 10 sessions complete karo.',
    },
  },
  ...
  'api-gateway': {
    1: {
      ...
      gotcha:
        'API Gateway SPOF trap: single gateway → single point of failure → entire system down. Solution: horizontal scaling of gateway, multiple availability zones, health checks. Another trap: putting business logic in gateway — gateway should be infrastructure, not application. Transformation logic, validation rules belong in services. Gateway fat = deployment coupling. FAANG interview: "What should NOT go in an API Gateway?" — business logic, complex orchestration, heavy computation.',
      closing:
        'API Gateway is infrastructure, not business logic layer. Cross-cutting concerns — auth, rate limiting, routing, logging — yes. Business rules — no. Design for horizontal scaling. Guru-sishya.in pe gateway architecture session hai.',
    },
    2: {
      ...
      gotcha:
        'Fixed window boundary attack: rate limit 100 req/minute. Attacker sends 100 requests at 0:59 aur 100 at 1:01 — 200 requests in 2 seconds but both windows show exactly 100. Solution: sliding window algorithm. Distributed rate limiting race condition: two gateway instances simultaneously check counter, both see 99, both allow request — 101st request sneaks through. Solution: Redis Lua script for atomic check-and-increment, or Redis INCR return value use karo (if return > limit, reject). FAANG: "How do you implement distributed rate limiting without race conditions?"',
      closing:
        'Token bucket for burst-allowed APIs, sliding window for precise rate control, distributed Redis counter for multi-instance gateways. Rate limiting logic must be atomic. Guru-sishya.in pe rate limiting implementation labs hain.',
    },
    3: {
      ...
      gotcha:
        'JWT expiry trap: long-lived JWT tokens — user account disabled kiya but token still valid for 24 hours. Solution: short expiry (15 min) + refresh tokens, or maintain token blacklist (but then stateful). Better: token introspection — gateway calls auth service to validate token — adds latency but real-time revocation. FAANG: "How do you implement immediate JWT revocation?" — token blacklist in Redis (check per request), short expiry + refresh token rotation, or opaque tokens with token store.',
      closing:
        'JWT for stateless auth, OAuth2 for delegated authorization, mTLS for service mesh. Token revocation is the hard problem — short expiry + refresh tokens is production best practice. Guru-sishya.in pe auth flow diagrams hain.',
    },
    4: {
      ...
      gotcha:
        'Canary routing consistency trap: user hits new version (5% canary), makes a change, next request goes to old version (95% stable) — inconsistency. Solution: sticky sessions for canary — once user hits canary, always route to canary. Cookie ya header mein canary flag store karo. FAANG: "How do you ensure a user consistently hits the same deployment during canary rollout?" Sticky routing by user_id hash — same user, same version. This is the nuance most candidates miss.',
      closing:
        'Request routing = path + header + host + weight based. Canary requires sticky routing for consistency. Feature-flag-driven routing decouples deploy from release. Guru-sishya.in pe routing configuration lab hai.',
    },
    5: {
      ...
      gotcha:
        'Sunset header trap: deprecated API pe `Sunset: Sat, 31 Dec 2025 23:59:59 GMT` header return karo — clients should read this and migrate. Reality: clients ignore headers. Solution: aggressive communication — email, docs, status page, error messages in response body warning about deprecation. Also: traffic monitoring — agar koi v1 use kar raha hai 1 day before sunset, proactively contact them. FAANG: "How do you safely retire an API version?" — sunset header + monitoring + client communication plan.',
      closing:
        'URL versioning for external APIs (explicit), header versioning for internal APIs. Additive-only changes to minimize version count. Sunset + monitor + communicate for safe deprecation. Guru-sishya.in pe API lifecycle management dekho.',
    },
    6: {
...
function buildStoryboard(topic: TopicConfig, session: SessionConfig): Storyboard {
  ...
  const narrationList = [narrations.hook, narrations.body, narrations.gotcha, narrations.closing];
