...
const txtPath = path.join(
  outputDir,
  `dialogue-${topic.replace(/[^a-z0-9]/gi, '-')}-s${sessionNumber}.txt`
);
const txtContent =
...
const srtPath = path.join(
  outputDir,
  `dialogue-${topic.replace(/[^a-z0-9]/gi, '-')}-s${sessionNumber}.srt`
);
...
const jsonPath = path.join(
  outputDir,
  `dialogue-${topic.replace(/[^a-z0-9]/gi, '-')}-s${sessionNumber}.json`
);
const jsonContent = {
  topic,
  sessionNumber,
...
console.log(
  `Total duration: ${jsonContent.totalDurationSeconds.toFixed(1)}s (${(jsonContent.totalDurationSeconds / 60).toFixed(1)} min)`
);
console.log(`\nFiles created:`);
