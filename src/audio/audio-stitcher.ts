...
// ---------------------------------------------------------------------------
function generateRoomTone(durationSec: number, outputPath: string, roomTonePath: string): void {
...
// ---------------------------------------------------------------------------
export async function stitchAudio(scenes: TTSResult[], opts: StitchOptions): Promise<StitchResult> {
  const { outputDir, platform = 'youtube', bgmFile, silenceDurationMs = 800 } = opts;
  ...
  const concatList = processedPaths.map((p) => `file '${p.replace(/'/g, "'\\''")}'`).join('\n');
  ...
  const pass1Result = spawnSync(
    ...
    [
      ...
      rawMasterPath,
      '-af',
      `loudnorm=I=${target.lufs}:LRA=11:TP=${target.truePeak}:print_format=json`,
      '-f',
      'null',
      '-',
    ],
  ...
  if (!jsonMatch) {
    throw new Error(
      `[audio-stitcher] loudnorm pass-1 JSON parse failed.\nstderr: ${stderrText.slice(-800)}`
    );
  }
