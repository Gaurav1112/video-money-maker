...
export const PHRASE_MAP: ReadonlyArray<[string | RegExp, string]> = [
  // ── Hooks & openers ────────────────────────────────────────────────────
  [/^Today we('re going to| will) (learn|understand|explore|cover)/i, 'Aaj hum samjhenge'],
  [/^In this video[,]? (we('ll| will)|I('ll| will))/i, 'Is video mein hum'],
  [/^Let('s| us) (start|begin) (with|by)/i, 'Chaliye shuru karte hain'],
  [/^Before we (start|begin|dive in)/i, 'Shuru karne se pehle'],
  [/^Have you ever wondered/i, 'Kabhi socha hai aapne'],
  [
    /^Most (engineers|developers|people) (get this wrong|don't know|don't understand)/i,
    'Zyaadatar developers ye galat samajhte hain',
  ],
  [/^The number one (mistake|problem|issue)/i, 'Sabse badi galti'],
  [/^Here's the thing[:.]/i, 'Baat ye hai ki'],
  [/^Here is the thing[:.]/i, 'Baat ye hai ki'],

  // ── Explanation connectives ────────────────────────────────────────────
  [/\bthink of (it|this) as\b/gi, 'ise aise samjho'],
  [/\bthink of (it|this) like\b/gi, 'ye bilkul aise hai jaise'],
  [/\bfor example[,]?\b/gi, 'for example'], // keep English — universally understood
  [/\bin other words\b/gi, 'matlab ye hai ki'],
  [/\bto put it simply\b/gi, 'simple bhasha mein'],
  [/\blet me explain\b/gi, 'main explain karta hoon'],
  [/\bthe key (point|idea|insight) (here |is )/gi, 'sabse important baat ye hai ki '],
  [/\bthe bottom line (is|here is)\b/gi, 'conclusion ye hai ki'],
  [/\bwhat this means (is|for us)\b/gi, 'iska matlab hai ki'],
  [/\bnow, (let's|let us)\b/gi, 'ab chaliye'],
  [/\bso, (what|how|why)\b/gi, 'toh, $2'],

  // ── Common teaching phrases ────────────────────────────────────────────
  [/\bYou might be wondering\b/gi, 'Aap soch rahe honge'],
  [/\bGreat question[.!]/gi, 'Bahut accha sawaal hai!'],
  [/\bRemember,?\s*/gi, 'Yaad rakho, '],
  [/\bPro tip[:.]\b/gi, 'Pro tip:'],
  [/\bWatch out for\b/gi, 'Dhyan rakhna'],
  [/\bThis is (very |really |super )?important\b/gi, 'Ye bahut important hai'],
  [/\bAt the end of the day\b/gi, 'Aakhir mein'],
  [/\bLet's take a step back\b/gi, 'Ek baar peeche jaate hain'],
  [/\bImagine you('re| are)\b/gi, 'Socho aap'],
  [/\bImagine (a|an|the)\b/gi, 'Socho ek'],

  // ── Evaluative phrases ─────────────────────────────────────────────────
  [/\bThis is (a )?(really |very |super )?simple\b/gi, 'Ye bilkul simple hai'],
  [/\bThis is (a )?(really |very |super )?complex\b/gi, 'Ye thoda complex hai'],
  [/\bThis is (a )?(really |very |super )?powerful\b/gi, 'Ye bahut powerful concept hai'],
  [
    /\bThis is (a )?(really |very |super )?common (mistake|issue|bug)\b/gi,
    'Ye ek bahut common $3 hai',
  ],
  [/\bEasy, right\?/gi, 'Simple hai na?'],
  [/\bMakes sense\?/gi, 'Samajh aaya?'],
  [/\bDoes that make sense\?/gi, 'Ye samajh aaya?'],

  // ── Sequence markers ───────────────────────────────────────────────────
  [/\bFirst[,]? (of all[,]? )?let('s| us)\b/gi, 'Pehle'],
  [/\bSecond[,]? (let's|let us)\b/gi, 'Dusra'],
  [/\bThird[,]? (let's|let us)\b/gi, 'Teesra'],
  [/\bFinally[,]\b/gi, 'Aakhir mein,'],
  [/\bTo summarize[,:]?\b/gi, 'Summary yeh hai ki'],
  [/\bTo recap[,:]?\b/gi, 'Recap karte hain —'],
  [/\bLet's (now |)move on to\b/gi, 'Ab chalte hain'],
  [/\bMoving on[,]\b/gi, 'Aage chalte hain,'],
  [/\bBefore (we |)wrap up\b/gi, 'Khatam karne se pehle'],
  [/\bThat's (it|all) for (today|this video)[.!]?/gi, 'Bas, itna hi aaj ke liye!'],

  // ── Interview / FAANG prep hooks (high-value for Indian audience) ──────
  [/\bIn (a |your |an )?interview[,]?\b/gi, 'Interview mein'],
  [/\bFor (your |the )?interview[,]?\b/gi, 'Interview ke liye'],
  [/\bIf (an |your )?interviewer asks\b/gi, 'Agar interviewer pooche'],
  [/\bWhen (you |the )?interviewer asks\b/gi, 'Jab interviewer pooche'],
  [
    /\bThis (is|comes up) (very )?(often|frequently) in interviews\b/gi,
    'Ye interview mein bahut baar aata hai',
  ],
  [/\bA common interview question (is|asks)\b/gi, 'Ek common interview question hai —'],
  [/\bYour (FAANG|tech|software) (interview|prep)\b/gi, 'aapka $1 interview'],

  // ── Desi-specific encouragement ────────────────────────────────────────
  [/\bDon't worry,?\s*/gi, 'Ghabraao mat, '],
  [/\bYou('ve| have) got this[.!]/gi, 'Aap kar sakte ho!'],
  [/\bTrust me,?\s*/gi, 'Mujh par bharosa rakho, '],
  [/\bBelieve me,?\s*/gi, 'Main bol raha hoon, '],
  [/\bThis will (really |definitely )?(help|save) you\b/gi, 'Ye aapke bahut kaam aayega'],
];
 ...
 */
export function rewriteScenes(scenes: SceneInput[], options: RewriteOptions = {}): SceneInput[] {
