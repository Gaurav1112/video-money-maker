...
export const QUIZ_BANK: QuizQuestion[] = [
  // ── KAFKA (12 questions) ──────────────────────────────────────────
  {
    ...
    correctIndex: 1,
    explanation:
      'The answer is B — gone forever. acks=0 means fire and forget. Your producer does not even wait for confirmation. Most production systems lose data this way and do not know for months. LinkedIn processes 7 trillion messages per day and every single one uses acks=all.',
    twist:
      'The scary part? acks=0 is not even the default. acks=1 is — and that is ALSO unsafe if the leader crashes before replication.',
  ...
  # Survives broker crash`,
    ...
    spokenCode:
      'Look at the producer config on the left. acks equals zero means fire and forget — the broker never confirms it received your message. Retries equals zero means no second chance. On the right, the production-grade version. acks equals all forces every in-sync replica to acknowledge. min insync replicas equals two guarantees at least two brokers have the data before success. Three retries handles transient network blips. One line of config separates silent data loss from durability.',
    spokenExample:
      'LinkedIn invented Kafka. They process seven trillion messages per day. Every single production topic at LinkedIn uses acks equals all with min insync replicas equals two. They learned the hard way — in the early days a single broker crash silently dropped millions of profile updates. Today their config survives an entire data center going dark. This is the difference between a startup losing user data and a hyperscaler guaranteeing zero loss.',
    spokenCTA:
      'So — are you running acks equals all in production, or are you still on the unsafe default? Drop your config in the comments and grab the full Kafka deep dive at www dot guru dash sishya dot in.',
  },
  {
    topic: 'kafka',
    ...
    correctIndex: 1,
    explanation:
      'Messages get reprocessed. Kafka tracks consumer progress via offsets. If you crash before committing, the next consumer in the group picks up from the last committed offset — replaying everything. This is called at-least-once delivery. Uber lost trip data in 2019 because of this exact issue.',
    twist:
      'Here is the part nobody tells you: auto-commit is ON by default. Every 5 seconds. So your "safe" consumer is actually committing offsets for messages you have not finished processing.',
...
consumer.commitSync();  // safe`,
    ...
    spokenCode:
      'Look at the consumer code on the left. enable auto commit equals true is the dangerous default. Every five seconds Kafka silently moves the offset forward — even for messages your code has not finished processing. If you crash in that window, those records are gone. On the right, the safe pattern. Auto commit off. Poll, process, then explicitly commitSync. The offset only advances after your business logic succeeds.',
    spokenExample:
      'Uber lost trip data in 2019 because of this exact bug. A consumer crashed mid-batch after auto-commit had already advanced the offset. The replacement consumer started reading from a position past unprocessed trips. Thousands of rides never made it into billing. The post-mortem named auto commit as the root cause. Every payments-grade Kafka consumer at Uber now uses manual commitSync — the same one-line fix you see on the right.',
    spokenCTA:
      'So — is your consumer manually committing, or are you trusting the five-second auto-commit timer? Comment your setup and grab the full consumer guide at www dot guru dash sishya dot in.',
  },
  {
    topic: 'kafka',
    ...
    options: [
      'At least 1 replica must confirm',
      'Only the leader confirms',
      'All replicas confirm',
    ],
    correctIndex: 1,
    explanation:
      'It means only the leader needs to be alive. If the leader crashes after acknowledging but before replicating, your message is gone. LinkedIn sets min.insync.replicas=2 and acks=all on every production topic. Three config lines separate data safety from data loss.',
    twist:
      'Most Kafka tutorials on YouTube teach you the WRONG defaults. They show you acks=1 and never mention min.insync.replicas.',
...
# survives 1 broker loss`,
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'Kafka only guarantees ordering within a single partition. Messages across partitions have no ordering guarantee. This is the number one mistake candidates make in system design interviews. The interviewer wants to hear: use a single partition key for related events.',
    twist:
      'Here is the follow-up that trips people: if you add more partitions for throughput, you lose ordering for existing keys because of partition reassignment.',
  ...
  },
  {
    ...
    spokenHook:
      'Netflix processes seven trillion messages per day through Kafka. Here is the config that makes it possible.',
    ...
    correctIndex: 2,
    explanation:
      'At Netflix scale, network bandwidth is the bottleneck. Kafka is designed to be disk-sequential which makes I/O fast. But replicating 7 trillion messages across brokers saturates network links. Netflix solved this by putting Kafka brokers on dedicated 25Gbps network interfaces.',
    twist:
      'Most teams optimize Kafka for disk speed. Netflix engineers told me: the disk is never the problem. The network always is.',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation:
      'Rebalancing redistributes partitions across consumers in the group. When a consumer joins or leaves, Kafka reassigns partitions to maintain even distribution. The problem: during rebalance, ALL consumers stop processing. This can take 30 seconds to several minutes.',
    twist:
      'The fix most teams miss: incremental cooperative rebalancing. One config flag and your rebalance goes from 30 seconds of downtime to zero.',
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation:
      'Kafka can achieve exactly-once semantics but ONLY with idempotent producers enabled plus transactional APIs. The default is at-least-once. Most candidates say "Kafka is exactly-once" without knowing the configuration required. Interviewers at Amazon and Google specifically test this nuance.',
    twist:
      "The real interview insight: exactly-once is between producer and broker only. End-to-end exactly-once requires your consumer to be idempotent too. That part is YOUR job, not Kafka's.",
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'New messages get rejected with a NotEnoughReplicasException. Kafka does NOT automatically delete old messages when disk is full. The retention policy runs on a timer, not on disk pressure. Uber hit this during a surge event when log volume spiked 10x and filled the disks in 2 hours.',
    twist:
      'The fix is embarrassingly simple: set log.retention.bytes alongside log.retention.hours. Time-based retention alone is a ticking time bomb.',
  ...
  },
  {
    ...
    twist:
      'Here is the mic drop answer for interviews: Kafka is closer to a database than a queue. It has persistence, replication, and consumer offsets that act like cursors.',
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation:
      'Use a consistent partition key. All events for the same entity — same user, same order, same transaction — go to the same partition. Within that partition, ordering is guaranteed. This is the answer Amazon interviewers want: you do not need global ordering. You need entity-level ordering via partition keys.',
    twist:
      'The follow-up trap: "What if one partition gets all the traffic?" The answer: custom partitioner that spreads hot keys across sub-partitions with a sequence number for reassembly.',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'Consumer lag means your consumer cannot keep up with the rate of incoming messages. The lag number is the count of unprocessed messages. If it keeps growing, you will eventually hit retention limits and LOSE data. This is the number one operational metric for Kafka that most teams ignore.',
    twist:
      'The counterintuitive fix: adding more consumers does NOT always help. If you have fewer consumers than partitions, add consumers. If you have equal consumers and partitions, you need to optimize processing speed or add partitions.',
  ...
  },
  {
    ...
    spokenHook:
      'LinkedIn built Kafka in 2010 because no existing system could solve this one problem.',
    question: 'Why did LinkedIn build Kafka instead of using RabbitMQ?',
    options: [
      'RabbitMQ was too slow',
      'They needed replay capability',
      'RabbitMQ was too expensive',
    ],
    correctIndex: 1,
    explanation:
      'LinkedIn needed the ability to REPLAY messages. Traditional message queues delete messages after delivery. LinkedIn needed multiple teams to independently consume the same activity feed data at their own pace. This is the fundamental insight: Kafka decouples producers from consumers with a persistent log that any number of consumers can read.',
    twist:
      'RabbitMQ is actually FASTER than Kafka for single-consumer scenarios. Kafka wins when you have multiple consumers reading the same stream. If you only have one consumer, RabbitMQ is the better choice.',
  ...
  // ── API GATEWAY (6 questions) ─────────────────────────────────────
  {
    ...
    correctIndex: 1,
    explanation:
      'An API Gateway is a single entry point that handles cross-cutting concerns: authentication, rate limiting, logging, request routing, and protocol translation. It is NOT a load balancer. Google, Amazon, and Netflix all have dedicated API Gateways separate from their load balancers. Confusing the two is the fastest way to fail a system design interview.',
    twist:
      'Hot take: most startups do NOT need an API Gateway. A simple reverse proxy with 20 lines of nginx config does 90% of what Kong or AWS API Gateway does. The API Gateway industry is built on over-engineering.',
...
gateway:
  ...
  - request routing -> services`,
    ...
    spokenCode:
      'On the left, the wrong way. Every microservice ships its own auth code, its own rate limiter, its own logging. Duplicated across every team. When the auth library has a CVE, you patch it ten times. On the right, the gateway pattern. One place handles JWT verification, token bucket rate limiting, logging and tracing, and request routing. Services receive a clean pre-authenticated request and focus only on business logic.',
    spokenExample:
      'Netflix runs Zuul as their API Gateway and it handles two billion requests a day before a single packet touches a business microservice. When they had a critical OAuth vulnerability in 2018, they patched it in one place in Zuul and every downstream service was protected within minutes. Teams without an API Gateway took weeks to roll out the same fix across hundreds of services. This is why every FAANG system design interview expects you to draw an API Gateway.',
    spokenCTA:
      'So — does your team own cross-cutting concerns in a gateway, or are they duplicated across every service? Drop your answer in the comments and check out the full microservices course at www dot guru dash sishya dot in.',
  },
  {
    topic: 'api-gateway',
    ...
    twist:
      'Netflix open-sourced Zuul but then replaced it internally with a custom solution because Zuul could not handle their scale. The open source version is literally the version they STOPPED using.',
  ...
  },
  {
    ...
    spokenHook:
      'Stripe rejected me because I could not explain API Gateway rate limiting properly.',
    ...
    twist:
      'The advanced answer Stripe wants: you need BOTH. Gateway-level for global protection, and service-level for tenant isolation. One without the other leaves a gap.',
...
// rejected before reaching service`,
  ...
  },
  {
    ...
    options: [
      ...
      'Gateway is north-south, Mesh is east-west',
      'Mesh is for external traffic only',
    ],
    correctIndex: 1,
    explanation:
      'API Gateway handles north-south traffic — requests coming from external clients into your system. Service Mesh handles east-west traffic — communication between internal microservices. Istio, Linkerd, and Consul Connect are service meshes. Kong, AWS API Gateway, and Zuul are API Gateways. Google Cloud uses both: Cloud Endpoints for the gateway and Istio for the mesh.',
    twist:
      'Here is what trips senior engineers: some API Gateways like Kong now include service mesh features. The boundary is blurring. In 2 years this distinction might not exist.',
  ...
  },
  {
    ...
    spokenHook:
      'This one API Gateway pattern saved an Amazon team over two million dollars a year.',
    ...
    correctIndex: 0,
    explanation:
      'Gateway Aggregation combines multiple backend calls into a single client request. Instead of the mobile app making 5 API calls to load a dashboard, the gateway makes all 5 calls internally and returns one merged response. This reduces mobile round trips from 5 to 1, cutting latency by 80% on slow networks. Amazon uses this for their product page — one API call assembles data from 15 different services.',
    twist:
      'The trap: if you aggregate too aggressively, your gateway becomes a monolith. Amazon calls this the "God Gateway" anti-pattern. The rule: aggregate for the client, never for business logic.',
...
// 1 round trip, 80% faster`,
  ...
  // ── LOAD BALANCING (5 questions) ──────────────────────────────────
  {
    ...
    twist:
      'The algorithm that beats everything for most use cases: least connections. Two words. It automatically routes to the server with the fewest active connections. No configuration needed.',
...
# one slow server tanks everyone`,
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'Layer 7 inspects HTTP headers, cookies, and URLs to make routing decisions. Route /api/users to the user service and /api/orders to the order service — that is L7. Layer 4 only sees IP and port — it is faster but dumb. Netflix uses L7 for its API traffic and L4 for internal service communication where speed matters more than routing intelligence.',
    twist:
      'The counterintuitive truth: L4 load balancers handle 10x more connections per second than L7. If you do not need content-based routing, L7 is wasting CPU cycles inspecting every packet.',
...
location /api/admin {
  ...
  },
  {
    ...
    spokenHook:
      'If you are using sticky sessions, you have fallen into a trap most architects never escape.',
    ...
    correctIndex: 1,
    explanation:
      'Sticky sessions tie a user to a specific server. If that server dies, the user loses their session. If one server gets all the power users, it becomes overloaded while others sit idle. You cannot add or remove servers without disrupting users. Amazon moved away from sticky sessions in 2012 by externalizing session state to Redis.',
    twist:
      'The dirty secret: most teams use sticky sessions because their app stores state in memory. The fix is not a better load balancer — it is making your app stateless. But nobody wants to refactor.',
...
// pod dies = LB routes elsewhere, fine`,
  ...
  },
  {
    ...
    twist:
      'Regular load balancing would scatter guild members across all nodes, requiring cross-node communication for every single message. Consistent hashing turns an O(N) broadcast into an O(1) local delivery.',
  ...
  },
  {
    ...
    spokenHook:
      'If your strategy is to index everything, you are making your database slower, not faster.',
    question: 'Why can too many indexes hurt database performance?',
    options: [
      'Indexes use too much RAM',
      'Every write must update every index',
      'Indexes corrupt data',
    ],
    correctIndex: 1,
    explanation:
      'Every INSERT, UPDATE, and DELETE must update every index on that table. A table with 10 indexes means every single write does 10 additional B-tree modifications. Shopify found that removing unused indexes on their orders table improved write throughput by 40 percent. Reads get faster with indexes, but writes get slower — linearly with the number of indexes.',
    twist:
      'The rule of thumb: if an index is used in less than 5 percent of your queries, drop it. PostgreSQL has pg_stat_user_indexes that shows exactly which indexes are unused. Most teams have never checked.',
...
DROP INDEX idx_orders_currency;`,
  ...
  },
  {
    ...
    twist:
      'The hot take that wins interviews: most teams that chose NoSQL regret it within 2 years. They trade schema flexibility for a nightmare of data consistency bugs. Start with PostgreSQL. Move to NoSQL only when PostgreSQL cannot handle your specific access pattern.',
  ...
  },
  {
    ...
    spokenHook:
      'This one N plus 1 query brought down a production database serving ten million users.',
    question: 'What is the N+1 query problem?',
    options: [
      'Running N queries in a loop instead of one JOIN',
      'Having N+1 database connections',
      'A query that returns N+1 rows',
    ],
    correctIndex: 0,
    explanation:
      "The N+1 problem: you fetch a list of 100 orders with 1 query, then fetch each order's items with 100 separate queries. That is 101 queries instead of 1 query with a JOIN. GitHub discovered N+1 queries were responsible for 70 percent of their slow pages. Each query is fast individually but 100 round trips to the database adds up to seconds of latency.",
    twist:
      'ORMs are the biggest N+1 offender. Rails Active Record, Django ORM, and Hibernate all make it trivially easy to write N+1 queries without realizing it. The ORM hides the SQL, which hides the problem.',
...
// 100x faster on real data`,
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'Soft delete marks records with a deleted_at timestamp instead of physically removing them. The data is still there if you need to recover it. Amazon, Stripe, and most fintech companies mandate soft deletes for audit trails and regulatory compliance. A hard DELETE is irreversible — if you delete the wrong records, your only hope is a backup that might be hours old.',
    twist:
      'The cost of soft deletes: your table grows forever. Stripe runs a background job that hard-deletes soft-deleted records older than 90 days. Without this, their largest tables would be 60 percent deleted rows — wasting storage and slowing queries.',
...
WHERE deleted_at IS NULL;`,
  ...
  // ── MICROSERVICES (5 questions) ───────────────────────────────────
  {
    ...
    spokenHook:
      'Amazon rejected me in the system design round because of this one microservices mistake.',
    ...
    correctIndex: 1,
    explanation:
      'Synchronous calls create tight coupling. If Service A calls Service B which calls Service C, and C goes down, the failure cascades back through B to A. Your entire system fails because ONE service is down. Amazon learned this when a single internal service outage took down the entire checkout flow in 2012. They now mandate async communication for non-critical paths.',
    twist:
      'The solution is not "make everything async." The real answer: use the circuit breaker pattern. When Service C fails, Service B returns a degraded response instead of failing. Netflix invented Hystrix for exactly this — and it saved them from thousands of cascading outages.',
  ...
  },
  {
    ...
    options: [
      ...
      'Moved from microservices back to monolith',
      'Adopted Kubernetes',
    ],
    correctIndex: 1,
    explanation:
    ...
    twist:
      'The lesson is NOT "monoliths are better." The lesson is: microservices are a SCALING solution, not a starting solution. If your team is under 50 engineers, a monolith is almost certainly the right choice.',
  ...
  },
  {
    ...
    spokenHook:
      'Distributed transactions across microservices are a lie that architects tell themselves.',
    ...
    twist:
      'Two-phase commit sounds correct but it is a death trap at scale. It locks resources across all services until everyone agrees. One slow service and your entire system freezes. Google abandoned 2PC for Sagas in their internal payment systems.',
  ...
  },
  {
    ...
    options: [
      ...
      'Central deployment queue',
    ],
    correctIndex: 1,
    explanation:
      "Database per service. Each microservice owns its data and exposes it only through APIs. No direct database access from other services. This means the user service can migrate from MySQL to PostgreSQL without any other team knowing or caring. Netflix enforces this rule strictly — if you access another team's database directly, your code review is rejected.",
    twist:
      "The hidden cost: data duplication. Netflix has the same user data copied across dozens of services. They accept this because the alternative — a shared database — means one team's bad query can take down everyone.",
  ...
  },
  {
    ...
    spokenHook:
      'This microservices pattern is the one that makes or breaks your system design interview.',
    question: 'What is the Strangler Fig pattern?',
    options: [
      'Gradually replace a monolith with microservices',
      'Kill failing microservices automatically',
      'Route traffic away from slow services',
    ],
    correctIndex: 0,
    explanation:
    ...
    twist:
      'Most teams try "big bang" rewrites and fail. The Strangler Fig works because you can stop at any point and still have a working system. If the new service is worse, just route traffic back. Zero risk.',
  ...
  // ── DOCKER (4 questions) ──────────────────────────────────────────
  {
    ...
    correctIndex: 1,
    explanation:
      'Multi-stage builds use one stage to compile your code and a second minimal stage to run it. The build tools, source code, and dependencies are left behind in the first stage. Google reduced their Go service images from 1.2GB to 12MB using multi-stage builds. The production image contains ONLY the compiled binary and nothing else.',
    twist:
      'Alpine seems like the answer but it uses musl libc instead of glibc. This causes subtle bugs in Node.js, Python, and Java applications. Google and Distroless images are safer choices for production than Alpine.',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'If an attacker escapes a container running as root, they get root access to the HOST machine. Container escapes are real — CVE-2019-5736 allowed exactly this in runc. Every major cloud provider was affected. The fix: add USER nonroot to your Dockerfile. Three words that prevent a catastrophic security breach. Stripe mandates non-root containers in every service.',
    twist:
      'The default Docker behavior is to run as root. Every Docker tutorial that does not include a USER instruction is teaching you to build insecure containers. That is most tutorials on YouTube.',
  ...
  },
  {
    ...
    options: [
      ...
      'Using too many RUN commands',
    ],
    correctIndex: 1,
    explanation:
      'If you COPY your entire source code before running npm install, EVERY code change invalidates the dependency cache. The fix: COPY package.json first, run npm install, THEN copy the source code. Dependencies are cached until package.json changes. Shopify cut their CI build times from 12 minutes to 2 minutes with this one reorder.',
    twist:
      'This applies to every language. Python: copy requirements.txt first. Go: copy go.mod first. Java: copy pom.xml first. The pattern is universal but most Dockerfile tutorials get the order wrong.',
  ...
  },
  {
    ...
    options: [
      'Never — Kubernetes is always better',
      'Single-host development and small deployments',
      'Only for testing',
    ],
    correctIndex: 1,
    explanation:
      'Docker Compose is for single-host deployments and local development. If your app runs on one server with under 10 containers, Compose is simpler and faster. Kubernetes is for multi-host orchestration with auto-scaling, self-healing, and rolling deployments. Using Kubernetes for a 3-container app is like driving a semi-truck to pick up groceries.',
    twist:
      'Here is the truth nobody admits: most startups that adopted Kubernetes too early spent more time managing Kubernetes than building their product. Basecamp runs a multi-million dollar business on Docker Compose. Not everything needs Kubernetes.',
  ...
  },
  {
    ...
    spokenHook:
      'Google rejected me because I could not explain the difference between a Deployment and a StatefulSet.',
    question: 'When should you use a StatefulSet instead of a Deployment in Kubernetes?',
    options: [
      'For any production workload',
      'When pods need stable network identity and persistent storage',
      'When you need more than 3 replicas',
    ],
    correctIndex: 1,
    explanation:
      'StatefulSets give each pod a stable hostname like mysql-0, mysql-1 and persistent storage that follows the pod across rescheduling. Databases, Kafka brokers, and ZooKeeper nodes need this because they must know WHO they are and WHERE their data is. A Deployment treats all pods as identical and interchangeable — perfect for stateless web servers, wrong for anything that stores data.',
    twist:
      'The expert answer: avoid running databases in Kubernetes entirely. Use managed services like RDS or Cloud SQL. Running PostgreSQL on Kubernetes is possible but you are now a database operator AND a Kubernetes operator. Most teams do not have the expertise for both.',
  ...
  },
  {
    ...
    options: [
      'It only checks every 30 seconds',
      'It waits for 3 minutes of sustained load',
      'It scales based on average, not peak CPU',
    ],
    correctIndex: 2,
    explanation:
    ...
    twist:
      'Even worse: HPA has a default cooldown of 5 minutes for scale-down. During a traffic spike, it scales up fast but then keeps those expensive pods running for 5 minutes after traffic drops. Your cloud bill thanks you.',
  ...
  },
  {
    ...
    spokenHook:
      'This one Kubernetes secret management mistake cost a startup half a million dollars.',
    ...
    options: [
      ...
      'They are visible to all namespaces',
    ],
    correctIndex: 1,
    explanation:
      'Kubernetes Secrets are base64 encoded, not encrypted. Anyone with etcd access or API access can read them in plain text. Base64 is an encoding, not encryption — it takes one command to decode. A startup stored their Stripe API keys in K8s Secrets, an engineer exported the cluster state for debugging, committed it to GitHub, and leaked production payment credentials. Cost: $500K in fraudulent charges.',
    twist:
      'The fix: enable etcd encryption at rest and use an external secret manager like HashiCorp Vault or AWS Secrets Manager. But here is the kicker: even with encryption, anyone with kubectl access can still read Secrets via the API. RBAC is your real security layer.',
  ...
  },
  {
    ...
    options: [
      ...
      'All clients request the same expired key simultaneously',
      'Redis runs out of memory',
    ],
    correctIndex: 1,
    explanation:
      'When a popular cache key expires, thousands of requests simultaneously miss the cache and hit the database. The database gets overwhelmed and crashes. Instagram hit this when a celebrity posted — the cached follower list expired and 50,000 requests per second hammered PostgreSQL. The fix: cache stampede protection. Only one request regenerates the cache while others wait or get a slightly stale value.',
    twist:
      'An even better fix: probabilistic early expiration. Each client randomly refreshes the cache slightly BEFORE it expires. This spreads the load over time instead of creating a cliff. Redis does not do this natively — you have to implement it in your application.',
  ...
  },
  {
    ...
    hookText: 'Redis as a\nDATABASE? Seriously?',
    spokenHook:
      'Companies are using Redis as a primary database. This is either genius or insanity.',
    question: 'Is Redis safe to use as a primary database?',
    options: [
      'Yes — Redis is durable by default',
      'No — Redis loses data on restart',
      'Only with AOF persistence enabled',
    ],
    correctIndex: 2,
    explanation:
      'Redis can be durable with AOF — Append Only File. Every write is logged to disk. With AOF set to fsync every second, you lose at most 1 second of data on a crash. Twitter uses Redis as a primary store for their ad targeting data. But most teams use Redis with default settings which means RDB snapshots every few minutes — a crash loses ALL data since the last snapshot.',
    twist:
      'The real risk is not data loss — it is cost. Redis stores everything in RAM. A 100GB dataset costs thousands per month in memory. The same data in PostgreSQL costs 10 dollars on disk. Use Redis for hot data only, not as a general-purpose database.',
  ...
  // ── SYSTEM DESIGN INTERVIEW (5 questions) ─────────────────────────
  {
    ...
    spokenHook:
      'This Design Twitter answer is the one that got me an L5 offer at a top tech company.',
    ...
    correctIndex: 1,
    explanation:
      "Fan-out on write pre-computes every user's timeline when a tweet is posted. Fan-out on read assembles the timeline at request time by querying all followed users. Twitter uses BOTH — fan-out on write for normal users and fan-out on read for celebrities with millions of followers. Pre-computing timelines for a celebrity with 50 million followers would be insanely expensive.",
    twist:
      'The follow-up that separates L4 from L5: "What about a celebrity retweeting another celebrity?" This creates a fan-out explosion. Twitter solved it with a hybrid approach and real-time ranking that most candidates never mention.',
  ...
  },
  {
    ...
    options: [
      ...
      'Handling redirect latency',
      'Storing billions of URLs',
    ],
    correctIndex: 0,
    explanation:
      'Generating globally unique short codes at scale without collisions is the core challenge. If two servers generate the same code simultaneously, one URL overwrites the other. Solutions: pre-generate ranges of IDs per server, use base62 encoding of a distributed counter like Snowflake IDs, or use a hash with collision resolution. Bit.ly generates over 600 million links and uses a counter-based approach for guaranteed uniqueness.',
    twist:
      'Most candidates jump to hashing the URL. But MD5 and SHA produce 128+ bit hashes that you truncate to 7 characters — creating collision probability. A counter is simpler, faster, and collision-free. Sometimes the boring solution is the right solution.',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'Token bucket allows bursts while maintaining an average rate. A bucket holds N tokens and refills at a fixed rate. Each request consumes a token. If the bucket is empty, the request is rejected. AWS, Stripe, and Google all use token bucket for their public APIs. It handles bursty traffic gracefully — unlike fixed window which allows double the rate at window boundaries.',
    twist:
      'The fixed window boundary problem: if your limit is 100 requests per minute and a client sends 100 requests at 0:59 and 100 more at 1:01, they sent 200 requests in 2 seconds but both windows show only 100. Sliding window or token bucket eliminates this loophole.',
  ...
  },
  {
    ...
    options: [
      ...
      'Presence tracking for 2B users',
    ],
    correctIndex: 2,
    explanation:
      'Tracking who is online right now across 2 billion users is the hardest part. Every time someone opens or closes the app, that event must propagate to all their contacts. With an average of 200 contacts, one user going online triggers 200 presence updates. Multiply by 500 million daily active users and you get 100 billion presence events per day. WhatsApp uses a heartbeat system with eventual consistency — your "last seen" might be 30 seconds stale.',
    twist:
      'The reason most candidates miss this: they focus on message delivery which is a solved problem with message queues. Presence is unsolved at scale — even WhatsApp sometimes shows wrong online status. That is a feature, not a bug.',
  ...
  },
  {
    ...
    options: [
      'One giant server farm',
      'CDN edge caching with tiered storage',
      'Peer-to-peer like BitTorrent',
    ],
    correctIndex: 1,
    explanation:
      'YouTube uses tiered storage with CDN edge caching. Popular videos are cached on CDN edge servers close to users — that is why trending videos load instantly. Older or less popular videos live on cheaper cold storage and take slightly longer to load. The key insight: 80 percent of views come from 20 percent of videos. By caching only the popular ones at the edge, YouTube serves most requests without touching origin servers.',
    twist:
      'YouTube also transcodes every video into dozens of formats and resolutions. A single 10-minute upload generates 20+ versions. The storage cost is not the original video — it is all the transcoded copies. This is why YouTube compression is so aggressive.',
  ...
  // ── REST API (4 questions) ────────────────────────────────────────
  {
    ...
    options: [
      ...
      'Statelessness + HATEOAS + uniform interface',
    ],
    correctIndex: 2,
    explanation:
      'True REST requires statelessness, a uniform interface, HATEOAS (links in responses that guide the client), and a layered system. Most APIs that call themselves RESTful are just JSON over HTTP. They use POST for everything, ignore status codes, and have zero hypermedia links. Roy Fielding, who invented REST, has publicly said most so-called REST APIs are not REST at all.',
    twist:
      'Here is the controversial truth: nobody actually implements HATEOAS. Not Google, not Amazon, not Stripe. The industry decided that "REST" means JSON plus HTTP methods plus good URL design. True REST is an academic ideal that adds complexity without much practical value for most APIs.',
  ...
  },
  {
    ...
    spokenHook:
      'Confusing PUT and PATCH has cost more candidates more job offers than any other API question.',
    ...
    correctIndex: 1,
    explanation:
      'PUT replaces the ENTIRE resource. If you PUT a user object without the email field, the email is deleted. PATCH updates only the fields you send. Stripe API uses PATCH for updates — you can update just the customer name without touching their payment methods. Sending a PUT to the same endpoint would wipe all fields you did not include.',
    twist:
      'The real interview trap: PUT is idempotent — calling it twice produces the same result. PATCH is NOT guaranteed to be idempotent. If your PATCH increments a counter, calling it twice increments twice. This nuance separates senior from junior candidates.',
  ...
  },
  {
    ...
    correctIndex: 0,
    explanation:
      'URL path versioning like /v1/users and /v2/users is the most practical approach. It is visible, cacheable, and easy to understand. Stripe, GitHub, and Google Maps all use URL versioning. Header versioning is technically "more RESTful" but makes debugging harder — you cannot test different versions by changing the URL in a browser. Practicality wins over purity.',
    twist:
      'The real hot take: the best versioning strategy is to NOT need versioning. Design your API to be backwards compatible from day one. Add new fields, never remove old ones. Stripe has maintained backwards compatibility since 2011 — their v1 API still works.',
  ...
  },
  {
    ...
    options: [
      'Always — GraphQL is newer and better',
      'When clients need flexible queries over complex data',
      'When you need maximum performance',
    ],
    correctIndex: 1,
    explanation:
      'GraphQL shines when different clients need different data shapes from the same API. A mobile app needs 3 fields while the web dashboard needs 30. With REST, you either over-fetch or create dozens of endpoints. GitHub switched to GraphQL because their integrators needed wildly different data subsets. But for simple CRUD APIs with predictable access patterns, REST is simpler and faster.',
    twist:
      'GraphQL has a hidden cost: every query can be a denial-of-service attack. A deeply nested query can join millions of rows. Facebook, who CREATED GraphQL, had to build query complexity analysis and depth limiting to prevent this. Most teams adopt GraphQL without these safeguards.',
  ...
  // ── AUTHENTICATION / JWT (4 questions) ────────────────────────────
  {
    ...
    correctIndex: 1,
    explanation:
      'JWTs are stateless — the server does not track them. Once issued, a JWT is valid until it expires, even if the user changes their password or gets banned. If a token is stolen, you cannot invalidate it. The attacker has access until expiration. Auth0 recommends short-lived access tokens of 15 minutes plus refresh tokens to mitigate this. Discord learned this the hard way when compromised tokens were used for days.',
    twist:
      'The "solution" most teams implement — a token blacklist — defeats the entire purpose of JWT. If you need to check a blacklist on every request, you might as well use server-side sessions. You just rebuilt sessions but worse.',
  ...
  },
  {
    ...
    options: [
      'No difference — same protocol',
      'OAuth is for authorization, OIDC adds authentication',
      'OIDC replaces OAuth',
    ],
    correctIndex: 1,
    explanation:
      "OAuth 2.0 is an AUTHORIZATION protocol — it grants access to resources. It was designed so apps could post to your Facebook wall, not to verify WHO you are. OpenID Connect adds an identity layer on top of OAuth that provides authentication — an ID token that proves the user's identity. Using raw OAuth for login is like using a car key to prove you own the house. Google and Microsoft both mandate OIDC for authentication.",
    twist:
      'Thousands of apps use "Sign in with Google" via plain OAuth without OIDC. This works but has a subtle vulnerability: the access token can be replayed. OIDC\'s ID token includes an audience claim that prevents this. If you are using OAuth for login, you probably have this vulnerability.',
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'bcrypt is designed to be SLOW — that is the point. A GPU can compute 10 billion SHA-256 hashes per second but only 50,000 bcrypt hashes. This makes brute force attacks impractical. The salt prevents rainbow table attacks by making each hash unique even for identical passwords. Dropbox uses bcrypt and was unaffected when 68 million password hashes were leaked in 2016 — because cracking bcrypt at scale is computationally infeasible.',
    twist:
      'SHA-256 is a great hash function but TERRIBLE for passwords because it is designed to be fast. Fast is the enemy of password security. The newer alternative to bcrypt is Argon2, which won the Password Hashing Competition in 2015 and also resists GPU attacks by requiring large amounts of memory.',
  ...
  },
  {
    ...
    spokenHook:
      'The session versus JWT debate is finally over. And the answer surprises most developers.',
    question: 'When should you use server-side sessions instead of JWT?',
    options: [
      'Never — JWT is always better',
  ...
  },
  {
    ...
    options: [
      'Always — canary is safer',
      'When you need to test with real production traffic',
    ...
    twist:
      'Blue-green has a hidden advantage: instant rollback. Just switch traffic back to the old version. Canary rollback requires draining connections from the canary. For stateful services, blue-green is actually safer. The answer depends on your specific system.',
  ...
  },
  {
    ...
    spokenHook:
      'Feature flags changed everything at Netflix and most teams still do not use them properly.',
    ...
    correctIndex: 1,
    explanation:
      'Feature flags let you deploy code to production with new features turned OFF. You deploy on Monday, test internally on Tuesday, enable for 10 percent of users on Wednesday, and fully release on Thursday. If anything breaks, flip the flag — no rollback, no redeploy. Netflix deploys hundreds of times per day because feature flags make every deployment safe. The code is live but the feature is dark until you are ready.',
    twist:
      'The dark side of feature flags: flag debt. Netflix has thousands of flags and some have been "temporary" for 3 years. Old flags create dead code paths, make testing harder, and slow down developers. LaunchDarkly recommends a lifecycle: create, activate, evaluate, retire. Most teams skip the retire step.',
 ...
 */
export function resolveTopicLock(date: Date, envValue: string | undefined): string | undefined {
 ...
 * crashing.
 */
export function getDailyQuiz(date: Date = new Date(), topicFilter?: string): QuizQuestion {
  const filter =
