...
function inferCategory(
  conceptName: string
): 'CODE' | 'PROCESS' | 'COMPARISON' | 'DATA' | 'CONCEPT' {
  const lower = conceptName.toLowerCase();

  if (lower.includes('code') || lower.includes('algorithm') || lower.includes('programming'))
    return 'CODE';
  if (
...
export const BROLL_CONFIG = {
  enableBRoll: true,
  brollPercentageOfScreen: 0.4, // Target: 40% screen
  maxClipsPerVideo: 15,
  overlayOpacity: 0.6,
  playbackRate: 0.8,
};
