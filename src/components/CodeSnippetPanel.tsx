...
const KEYWORDS: Record<string, RegExp> = {
  ...
  python:
    /\b(def|class|return|if|elif|else|for|while|try|except|raise|import|from|as|with|lambda|True|False|None|self|pass|yield|async|await)\b/g,
  yaml: /^\s*([\w.-]+)(?=\s*:)/gm,
  json: /"([^"\\]*(?:\\.[^"\\]*)*)"(?=\s*:)/g,
    ...
    >
      ...
      >
        {visible.map((tok, i) => (
          <span
            key={i}
            style={{ color: COLORS[tok.kind], fontWeight: tok.kind === 'keyword' ? 700 : 500 }}
          >
            {tok.text}
          </span>
        ))}
...
export const CodeSnippetPanel: React.FC<Props> = ({ snippet, startFrame, durationFrames }) => {
  ...
  const wrongTokens = useMemo(
    () => tokenize(snippet.wrong, snippet.language),
    [snippet.wrong, snippet.language]
  );
  const rightTokens = useMemo(
    () => tokenize(snippet.right, snippet.language),
    [snippet.right, snippet.language]
  );
      ...
      <>
        <div
          style={{
            ...
            right: 60,
            zIndex: 25,
            transform: `translateY(${wrongOff}px)`,
            opacity: wrongOp,
          }}
        >
          <CodeBlock
        ...
        <div
          style={{
            ...
            right: 60,
            zIndex: 25,
            transform: `translateY(${rightOff}px)`,
            opacity: rightOp,
          }}
        >
          <CodeBlock
