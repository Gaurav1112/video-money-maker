...
const POWER_WORD_RE =
  /\b(NOT|NEVER|WRONG|LOST|EVERY|ALWAYS|ALL|ONLY|MOST|CRITICAL|ZERO|NONE|TRILLION|BILLION|MILLION|FAIL|FAILED|CRASHED|SAVED|FIXED)\b|\$[\d,]+\s*[MB]?/g;
...
export const ExplanationBeats: React.FC<Props> = ({
  ...
  const sentenceIdx =
    ageAfterStat < 0 ? -1 : Math.min(sentences.length - 1, Math.floor(ageAfterStat / beatDur));
    ...
    >
      ...
      {sentenceIdx >= 0 &&
        (() => {
          ...
          const fadeOut = interpolate(beatAge, [beatDur - 10, beatDur], [1, 0.7], {
            extrapolateLeft: 'clamp',
            extrapolateRight: 'clamp',
          });
          return (
