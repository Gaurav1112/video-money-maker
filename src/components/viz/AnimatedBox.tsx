...
function getColorPreset(color?: string): { bg: string; border: string; icon: string } {
  ...
  if (c.includes('e85d26') || c.includes('saffron') || c.includes('ff9900') || c.includes('orange'))
    return COLOR_PRESETS.orange;
  if (c.includes('a78bfa') || c.includes('818cf8') || c.includes('purple') || c.includes('8e44'))
...
export const AnimatedBox: React.FC<AnimatedBoxProps> = ({
  ...
  return (
    ...
    >
      {IconComponent && <IconComponent size={32} color={preset.icon} strokeWidth={1.5} />}
      <span
        style={{
          fontSize: 28,
