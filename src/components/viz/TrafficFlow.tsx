...
const ClientDot: React.FC<ClientDotProps> = ({
  x,
  y,
  revealSpring,
  isActive,
  ...
  const pulse = isActive ? 1 + 0.18 * Math.sin(frame * 0.18) : 1;
...
const LoadBalancerBox: React.FC<LoadBalancerBoxProps> = ({
  springVal,
  isActive,
  frame,
  subtitle,
  ...
  return (
    <div
      style={{
        ...
        boxShadow: `0 0 ${24 * glowPulse}px ${C.saffron}${Math.floor(glowPulse * 160)
          .toString(16)
          .padStart(2, '0')}`,
        display: 'flex',
        alignItems: 'center',
...
const Arrow: React.FC<ArrowProps> = ({
  ...
  const pulseOpacity = pulseFrame !== undefined ? 0.5 + 0.3 * Math.sin(pulseFrame * 0.06) : 0.7;
...
const ServerBox: React.FC<ServerBoxProps> = ({
  x,
  y,
  label,
  springVal,
...
const HealthBar: React.FC<HealthBarProps> = ({
  ...
  return (
    ...
    >
      <span
        style={{ fontSize: 9, color: C.gray, fontFamily: 'Inter, sans-serif', letterSpacing: 0.5 }}
      >
        CPU
      </span>
      <div
...
export const TrafficFlow: React.FC<TrafficFlowProps> = ({ sync, frame, keywords, variant }) => {
  ...
  if (variant === 'overload')
    return <OverloadVariant sync={sync} frame={frame} keywords={keywords} />;
  if (variant === 'healthcheck')
    return <HealthcheckVariant sync={sync} frame={frame} keywords={keywords} />;
  ...
  const arrowDrawProgress = progressWindow(p, 0.5, 0.7);

  const serverRevealProgress = progressWindow(p, 0.3, 0.5);
  ...
  const healthFills = [0.42, 0.67, 0.31].map(
    (base, i) =>
      base * healthFillProgress +
      (healthFillProgress > 0 ? 0.06 * Math.sin(frame * 0.08 + i * 2) : 0)
  );

  // Flow starts as soon as servers appear — frame-driven, not progress-gated
  const flowingActive = p > 0.7;
  ...
  for (let lane = 0; lane < 3; lane++) {
    for (let dotIdx = 0; dotIdx < 2; dotIdx++) {
      const offset =
        lane * Math.floor(dotPeriod / 3) +
        dotIdx * Math.floor(dotPeriod / 2) +
        Math.floor(dotPeriod / 4);
      const rawT = ((frame - offset) % dotPeriod) / dotPeriod;
      ...
      >
        {clientToLbArrows.map((a, i) => (
          <Arrow
            ...
            color={C.gray}
            pulseFrame={flowingActive ? frame : undefined}
          />
        ))}
        ...
        {flowDots.map((dot, idx) => {
          ...
          return (
            <FlowingDot
              ...
              x2={a.x2}
              y2={a.y2}
              color={serverColors[dot.lane]}
              visible={flowingActive && serverRevealProgress > 0.3}
            />
          );
        })}
...
// =====================================================================
const OverloadVariant: React.FC<Omit<TrafficFlowProps, 'variant'>> = ({
  sync,
  frame,
  keywords,
}) => {
  ...
  const serverPos = { fx: 0.5, fy: 0.72 };
  const svgW = width;
  const svgH = height;
  ...
  const clientSprings = clientPositions.map((_, i) =>
    spring({
      ...
      config: { damping: 14, stiffness: 120, mass: 0.7 },
      from: 0,
      to: clientRevealP > 0 ? 1 : 0,
    })
  );

  const serverRevealP = progressWindow(p, 0.15, 0.3);
  const serverSpring = spring({
    frame,
    fps,
    config: { damping: 13, stiffness: 110, mass: 0.8 },
    from: 0,
    to: serverRevealP > 0 ? 1 : 0,
  });

  const arrowDrawP = progressWindow(p, 0.3, 0.5);

  // Overload builds up over most of the scene (starts at 50%)
  const overloadP = progressWindow(p, 0.5, 0.9);
  const healthFill = 0.15 + overloadP * 0.83;
  const serverStatus: 'healthy' | 'overloaded' | 'down' =
    healthFill > 0.88 ? 'down' : healthFill > 0.55 ? 'overloaded' : 'healthy';
  ...
  const clientArrows = clientPositions.map((c) => ({
    x1: c.fx * svgW,
    y1: (c.fy + 0.04) * svgH,
    x2: serverPos.fx * svgW,
    y2: (serverPos.fy - 0.06) * svgH,
  }));
  ...
  return (
    ...
    >
          ...
          for (let d = 0; d < 2; d++) {
            ...
            dots.push(
              <FlowingDot
                key={`fd-${i}-${d}`}
                t={t}
                x1={clientArrows[i].x1}
                y1={clientArrows[i].y1}
                x2={clientArrows[i].x2}
                y2={clientArrows[i].y2}
                color={C.saffron}
                visible={flowActive}
                size={4}
              />
      ...
      <HealthBar
        x={`${serverPos.fx * 100}%`}
        y={`${(serverPos.fy + 0.06) * 100}%`}
        fill={healthFill}
        color={healthFill > 0.7 ? C.red : C.teal}
        opacity={serverRevealP}
        pulsing
        frame={frame}
      />
      ...
      {overloadP > 0.3 && (
        <div
          style={{
            ...
            padding: '10px 24px',
            fontFamily: 'Inter, sans-serif',
            opacity: interpolate(overloadP, [0.3, 0.5], [0, 1], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            }),
          }}
        >
          <span style={{ fontSize: 16, fontWeight: 700, color: C.red }}>
            {serverStatus === 'down'
              ...
              : 'WARNING: CPU at ' + Math.round(healthFill * 100) + '%'}
          </span>
        </div>
      )}
    </div>
...
// =====================================================================
const HealthcheckVariant: React.FC<Omit<TrafficFlowProps, 'variant'>> = ({
  sync,
  frame,
  keywords,
}) => {
  ...
  const failureP = progressWindow(p, 0.55, 0.7);
  const downP = progressWindow(p, 0.7, 0.8);
  const rerouteP = progressWindow(p, 0.8, 1.0);
  ...
  const lbRevealP = progressWindow(p, 0.15, 0.3);
  const lbSpring = spring({
    frame,
    fps,
    config: { damping: 12, stiffness: 100, mass: 0.9 },
  ...
  const activeServerIdx = flowActive
    ? activeServers[Math.floor(frame / 20) % activeServers.length]
    : -1;
      ...
      >
            ...
            return (
              <g key={`hc-${i}`} opacity={pulseOpacity}>
                ...
                <text
                  ...
                  fontWeight={600}
                >
                  {i === 1 && server2Status === 'down'
                    ? 'FAIL'
                    : i === 1 && server2Status === 'overloaded'
                      ? 'SLOW'
                      : 'OK'}
                </text>
              </g>
          ...
          for (let d = 0; d < 2; d++) {
            ...
            dots.push(
              <FlowingDot
                key={`dot-${lane}-${d}`}
                t={t}
                x1={lbToServerArrows[lane].x1}
                y1={lbToServerArrows[lane].y1}
                x2={lbToServerArrows[lane].x2}
                y2={lbToServerArrows[lane].y2}
                color={serverColors[lane]}
                visible={flowActive && serverRevealP > 0.3}
              />
            );
          }
          return dots;
        ...
        {[0, 1, 2].map((i) => {
          ...
          return (
            <FlowingDot
              key={`cldot-${i}`}
              t={t}
              x1={clientToLbArrows[i].x1}
              y1={clientToLbArrows[i].y1}
              x2={clientToLbArrows[i].x2}
              y2={clientToLbArrows[i].y2}
              color={C.gray}
              visible={flowActive && serverRevealP > 0.3}
              size={4}
            />
        ...
        {server2Status === 'down' && (
          <g opacity={0.6 + 0.4 * Math.sin(frame * 0.1)}>
            <line
              x1={lbCenter.fx * svgW - 15}
              y1={(lbCenter.fy + 0.15) * svgH}
              x2={lbCenter.fx * svgW + 15}
              y2={(lbCenter.fy + 0.15) * svgH + 30}
              stroke={C.red}
              strokeWidth={3}
            />
            <line
...
const StickyVariant: React.FC<Omit<TrafficFlowProps, 'variant'>> = ({ sync, frame, keywords }) => {
  ...
  const serverRevealP = progressWindow(p, 0.4, 0.55);
  const serverSprings = [0, 1, 2].map((i) =>
    spring({
      frame: Math.max(0, frame - i * 5),
      fps,
  ...
  const affinityRevealP = progressWindow(p, 0.6, 0.75);
          ...
          for (let d = 0; d < 2; d++) {
            ...
            dots.push(
              <React.Fragment key={`sticky-${i}-${d}`}>
                ...
                <FlowingDot
                  t={t2}
                  x1={lbToServerArrows[i].x1}
                  y1={lbToServerArrows[i].y1}
                  x2={lbToServerArrows[i].x2}
                  y2={lbToServerArrows[i].y2}
                  color={affinityColors[i]}
                  visible={flowActive}
                  size={6}
                />
          ...
          [0, 1, 2].map((i) => {
            ...
            return (
              <text
                ...
                x={midX + 12}
                y={midY + bob}
                fill={affinityColors[i]}
                fontSize={10}
                fontWeight={600}
                fontFamily="Inter, sans-serif"
                opacity={affinityRevealP}
              >
                SID-{i + 1}
              </text>
            );
...
const ScaleVariant: React.FC<Omit<TrafficFlowProps, 'variant'>> = ({ sync, frame, keywords }) => {
  ...
  const visibleServerCount = p < 0.35 ? 2 : p < 0.55 ? 3 : p < 0.75 ? 4 : 5;
  ...
  const arrowDrawP = progressWindow(p, 0.3, 0.45);
    ...
    >
      ...
      >
        ...
        {activeServerPositions.map((_, i) => {
          ...
          for (let d = 0; d < 2; d++) {
            const offset =
              i * Math.floor(dotPeriod / activeServerPositions.length) +
              d * Math.floor(dotPeriod / 2);
            const rawT = ((frame - offset) % dotPeriod) / dotPeriod;
            const t = rawT < 0 ? rawT + 1 : rawT;
            dots.push(
              <FlowingDot
