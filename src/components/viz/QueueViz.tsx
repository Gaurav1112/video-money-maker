...
const MessageBox: React.FC<MessageBoxProps> = ({
  ...
  return (
    <g opacity={opacity}>
      ...
      />
      {isProcessing && (
        <rect x={x} y={y} width={width} height={height} rx={4} fill={color} opacity={pulse * 0.2} />
      )}
      <text
        x={x + width / 2}
        y={y + height / 2 + 1}
...
// =====================================================================
const ThroughputCounter: React.FC<{
  produced: number;
  consumed: number;
  frame: number;
  fps: number;
...
const FifoVariant: React.FC<Omit<QueueVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  // Layout
  const producerPos = { fx: 0.1, fy: 0.45 };
  const queuePos = { fx: 0.5, fy: 0.45 };
  const consumerPos = { fx: 0.9, fy: 0.45 };

  const revealP = progressWindow(p, 0, 0.1);
  ...
  const prodArrow = {
    x1: producerPos.fx * svgW + 50,
    y1: producerPos.fy * svgH,
    x2: queueX - 10,
    y2: queuePos.fy * svgH,
  };
  // Queue -> Consumer arrow
  const consArrow = {
    x1: queueX + queueWidth + 10,
    y1: queuePos.fy * svgH,
    x2: consumerPos.fx * svgW - 50,
    y2: consumerPos.fy * svgH,
  };
    ...
    >
      ...
      >
        ...
        <g opacity={arrowDrawP}>
          ...
          />
          <polygon
            points={`${prodArrow.x2},${prodArrow.y2} ${prodArrow.x2 - 8},${prodArrow.y2 - 5} ${prodArrow.x2 - 8},${prodArrow.y2 + 5}`}
            fill={C.saffron}
            opacity={0.8}
          />
          <text
            x={(prodArrow.x1 + prodArrow.x2) / 2}
            y={prodArrow.y1 - 14}
            fill={C.saffron}
            fontSize={10}
            fontWeight={600}
        ...
        <g opacity={arrowDrawP}>
          ...
          />
          <polygon
            points={`${consArrow.x2},${consArrow.y2} ${consArrow.x2 - 8},${consArrow.y2 - 5} ${consArrow.x2 - 8},${consArrow.y2 + 5}`}
            fill={C.teal}
            opacity={0.8}
          />
          <text
            x={(consArrow.x1 + consArrow.x2) / 2}
            y={consArrow.y1 - 14}
            fill={C.teal}
            fontSize={10}
            fontWeight={600}
      ...
      <div
        style={{
          position: 'absolute',
          left: `${producerPos.fx * 100}%`,
          top: `${producerPos.fy * 100}%`,
      ...
      <div
        style={{
          position: 'absolute',
          left: `${consumerPos.fx * 100}%`,
          top: `${consumerPos.fy * 100}%`,
          transform: `translate(-50%, -50%) scale(${nodeSpring})`,
          opacity: nodeSpring,
          width: 80,
          height: 60,
      ...
      {flowActive && (
        ...
        >
          <div style={{ textAlign: 'center' }}>
            ...
            <div
              style={{ fontSize: 20, fontWeight: 800, color: queueDepth > 4 ? C.saffron : C.teal }}
            >
              {queueDepth}
            </div>
          </div>
...
const PubSubVariant: React.FC<Omit<QueueVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  const subscribers = [
    ...
    { fx: 0.85, fy: 0.4, label: 'Analytics', color: C.gold },
  ...
  const subSprings = subscribers.map((_, i) =>
    spring({
      ...
      from: 0,
      to: progressWindow(p, 0.08, 0.18) > 0 ? 1 : 0,
    })
  );

  const arrowDrawP = progressWindow(p, 0.1, 0.2);
  const flowActive = p > 0.18;
  const dotPeriod = fps * 1.6;

  // Pub -> Topic arrow
  const pubArrow = {
    x1: publisherPos.fx * svgW + 50,
    y1: publisherPos.fy * svgH,
    x2: topicPos.fx * svgW - 60,
    y2: topicPos.fy * svgH,
  };

  // Topic -> Subscriber arrows (fan-out)
  const subArrows = subscribers.map((s) => ({
    x1: topicPos.fx * svgW + 60,
    y1: topicPos.fy * svgH,
    x2: s.fx * svgW - 50,
    y2: s.fy * svgH,
  }));
    ...
    >
      ...
      >
        ...
        <g opacity={arrowDrawP * (0.5 + 0.3 * Math.sin(frame * 0.06))}>
          ...
          />
          <polygon
            points={`${pubArrow.x2},${pubArrow.y2} ${pubArrow.x2 - 8},${pubArrow.y2 - 5} ${pubArrow.x2 - 8},${pubArrow.y2 + 5}`}
            fill={C.saffron}
          />
          <text
            x={(pubArrow.x1 + pubArrow.x2) / 2}
            y={pubArrow.y1 - 12}
            fill={C.saffron}
            fontSize={10}
            fontWeight={600}
          ...
          for (let d = 0; d < 3; d++) {
            ...
            dots.push(
              <circle
                ...
                r={5}
                fill={subscribers[i].color}
                opacity={flowActive ? alpha * 0.8 : 0}
            ...
            if (flowActive && t > 0.05) {
              ...
              dots.push(
                <circle
                  ...
                  r={3}
                  fill={subscribers[i].color}
                  opacity={flowActive ? alpha * 0.3 : 0}
                />
              );
            }
          }
        ...
        <g opacity={nodeSpring}>
          <rect
            x={topicPos.fx * svgW - 55}
            y={topicPos.fy * svgH - 35}
            width={110}
            height={70}
            rx={10}
            fill={`${C.gold}15`}
          ...
          {[0, 1, 2].map((i) => (
            <circle
              key={`bc-${i}`}
              cx={topicPos.fx * svgW}
              cy={topicPos.fy * svgH - 8}
              r={8 + i * 6}
              fill="none"
              stroke={C.gold}
              strokeWidth={1}
              opacity={0.15 + 0.1 * Math.sin(frame * 0.08 + i)}
            />
          ))}
          <text
            x={topicPos.fx * svgW}
            y={topicPos.fy * svgH + 18}
            fill={C.gold}
            fontSize={12}
            fontWeight={700}
            fontFamily="Inter, sans-serif"
      ...
      <div
        style={{
          position: 'absolute',
          left: `${publisherPos.fx * 100}%`,
          top: `${publisherPos.fy * 100}%`,
          transform: `translate(-50%, -50%) scale(${nodeSpring})`,
          opacity: nodeSpring,
          width: 80,
          height: 60,
      ...
      {subscribers.map((sub, i) => (
        <div
          ...
          style={{
            ...
            left: `${sub.fx * 100}%`,
            top: `${sub.fy * 100}%`,
            transform: `translate(-50%, -50%) scale(${subSprings[i]})`,
            opacity: subSprings[i],
            width: 90,
            height: 50,
            borderRadius: 8,
...
const DeadLetterVariant: React.FC<Omit<QueueVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  const producerPos = { fx: 0.1, fy: 0.35 };
  ...
  const dlqRevealP = progressWindow(p, 0.3, 0.4);
  const dlqSpring = spring({
    frame,
    fps,
    config: { damping: 13, stiffness: 110, mass: 0.8 },
    from: 0,
    ...
    >
      ...
      >
        ...
        {dlqSpring > 0 && (
          <g opacity={dlqSpring * (0.5 + 0.3 * Math.sin(frame * 0.06))}>
            <line
              x1={queuePos.fx * svgW}
              y1={queuePos.fy * svgH + queueH / 2 + 5}
              x2={dlqPos.fx * svgW}
              y2={dlqPos.fy * svgH - dlqH / 2 - 5}
              stroke={C.red}
              strokeWidth={2}
              strokeLinecap="round"
              strokeDasharray="6 4"
            />
            <text
              x={queuePos.fx * svgW - 60}
              y={(queuePos.fy * svgH + dlqPos.fy * svgH) / 2}
              fill={C.red}
              fontSize={10}
              fontWeight={600}
              fontFamily="Inter, sans-serif"
        ...
        <g opacity={dlqSpring}>
          <rect
            x={dlqPos.fx * svgW - dlqW / 2}
            y={dlqPos.fy * svgH - dlqH / 2}
            width={dlqW}
            height={dlqH}
            rx={8}
            fill={`${C.red}12`}
          ...
          {Array.from({ length: Math.min(3, failCount) }).map((_, i) => (
            <g key={`dlq-${i}`}>
              <rect
                x={dlqPos.fx * svgW - dlqW / 2 + 10 + i * 56}
                y={dlqPos.fy * svgH - 10}
                width={50}
                height={20}
                rx={3}
              ...
              {/* Skull icon */}
              <text
                x={dlqPos.fx * svgW - dlqW / 2 + 22 + i * 56}
                y={dlqPos.fy * svgH + 4}
                fill={C.red}
                fontSize={10}
                fontFamily="Inter, sans-serif"
              ...
              <text
                x={dlqPos.fx * svgW - dlqW / 2 + 42 + i * 56}
                y={dlqPos.fy * svgH + 4}
                fill={C.red}
                fontSize={7}
                fontWeight={600}
        ...
        {flowActive && isFailing && (
          <g>
            <circle
              cx={consumerPos.fx * svgW + shakeX}
              cy={consumerPos.fy * svgH + shakeY}
              r={40 + 10 * Math.sin(frame * 0.15)}
              fill="none"
              stroke={C.red}
              strokeWidth={2}
              opacity={0.3 + 0.3 * Math.sin(frame * 0.15)}
            />
            <circle
              cx={consumerPos.fx * svgW + shakeX}
              cy={consumerPos.fy * svgH + shakeY}
              r={50 + 8 * Math.sin(frame * 0.12)}
              fill="none"
              stroke={C.red}
              strokeWidth={1}
              opacity={0.15 + 0.15 * Math.sin(frame * 0.12)}
            />
          </g>
        )}
        ...
        {flowActive &&
          (() => {
            ...
            if (t < 0.4) {
              ...
              const eased = t / 0.4;
              const cx =
                producerPos.fx * svgW + 50 + (queueX - 10 - producerPos.fx * svgW - 50) * eased;
              return (
                <circle
                  cx={cx}
                  cy={producerPos.fy * svgH}
                  r={5}
                  fill={C.saffron}
                  opacity={0.8}
                  style={{ filter: `drop-shadow(0 0 4px ${C.saffron})` }}
            ...
            if (t > 0.5 && t < 0.85) {
              ...
              const cx =
                queueX + queueW + 10 + (consumerPos.fx * svgW - 50 - queueX - queueW - 10) * eased;
      ...
      <div
        style={{
          ...
          left: `${consumerPos.fx * 100}%`,
          top: `${consumerPos.fy * 100}%`,
          transform: `translate(calc(-50% + ${shakeX}px), calc(-50% + ${shakeY}px)) scale(${nodeSpring})`,
          opacity: nodeSpring,
          width: 80,
          height: 55,
          borderRadius: 10,
      ...
      >
        <span style={{ fontSize: 12, fontWeight: 700, color: isFailing ? C.red : C.teal }}>
          CONSUMER
        </span>
      ...
      {dlqSpring > 0 && (
        <div
          style={{
            position: 'absolute',
            left: `${retryPos.fx * 100}%`,
            top: `${retryPos.fy * 100}%`,
            transform: `translate(-50%, -50%) scale(${dlqSpring})`,
            opacity: dlqSpring,
            width: 90,
            height: 50,
            borderRadius: 8,
