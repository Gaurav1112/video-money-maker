...
const DbNode: React.FC<DbNodeProps> = ({
  ...
  return (
    ...
    >
      ...
      {sublabel && (
        <span style={{ fontSize: 9, color: C.gray, letterSpacing: 0.5 }}>{sublabel}</span>
      )}
      {isDead && (
        <div
          style={{
...
const ReplicationVariant: React.FC<Omit<DatabaseVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  const clientPos = { fx: 0.05, fy: 0.2 };

  // Reveals — SPREAD across full 0-1 scene progress
  const primaryRevealP = progressWindow(p, 0, 0.2);
  const primarySpring = spring({
    frame,
    fps,
    config: { damping: 12, stiffness: 100, mass: 0.9 },
    from: 0,
    to: primaryRevealP > 0 ? 1 : 0,
  });

  const replicaRevealP = progressWindow(p, 0.2, 0.4);
  const replicaSprings = replicas.map((_, i) =>
    spring({
      ...
      from: 0,
      to: replicaRevealP > 0 ? 1 : 0,
    })
  );

  const arrowDrawP = progressWindow(p, 0.4, 0.6);
  const flowActive = p > 0.6;
  const dotPeriod = fps * 1.8;

  // Write arrows: client -> primary
  const writeArrow = {
    x1: clientPos.fx * svgW + 40,
    y1: clientPos.fy * svgH,
    x2: primary.fx * svgW - 60,
    y2: primary.fy * svgH,
  };

  // Replication arrows: primary -> each replica
  const repArrows = replicas.map((r) => ({
    x1: primary.fx * svgW + 60,
    y1: primary.fy * svgH,
    x2: r.fx * svgW - 50,
    y2: r.fy * svgH,
  }));
  ...
  const readArrows = replicas.map((r, i) => ({
    x1: r.fx * svgW + 50,
    y1: r.fy * svgH,
    x2: readClientPositions[i].fx * svgW,
    y2: readClientPositions[i].fy * svgH,
  }));
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          position: 'absolute',
          left: `${clientPos.fx * 100}%`,
          top: `${clientPos.fy * 100}%`,
      ...
      {replicas.map((r, i) => (
        <DbNode
          ...
          height={80}
          borderColor={C.teal}
          springVal={replicaSprings[i]}
          isActive={activeReplicaIdx === i}
          frame={frame}
        />
      ))}
...
const ShardingVariant: React.FC<Omit<DatabaseVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  const routerPos = { fx: 0.5, fy: 0.2 };
  const shards = [
    { fx: 0.2, fy: 0.65, label: 'SHARD A', range: 'Users A-H', color: C.saffron },
    { fx: 0.5, fy: 0.7, label: 'SHARD B', range: 'Users I-P', color: C.gold },
    { fx: 0.8, fy: 0.65, label: 'SHARD C', range: 'Users Q-Z', color: C.teal },
  ];

  const routerRevealP = progressWindow(p, 0, 0.2);
  const routerSpring = spring({
    frame,
    fps,
    config: { damping: 12, stiffness: 100, mass: 0.9 },
    from: 0,
    to: routerRevealP > 0 ? 1 : 0,
  });

  const shardRevealP = progressWindow(p, 0.2, 0.4);
  ...
  const arrows = shards.map((s) => ({
    x1: routerPos.fx * svgW,
    y1: routerPos.fy * svgH + 40,
    x2: s.fx * svgW,
    y2: s.fy * svgH - 50,
  }));
  ...
  return (
    ...
    >
          ...
          for (let d = 0; d < 2; d++) {
            ...
            dots.push(
              <DataPacket
                ...
                y2={a.y2}
                color={shards[i].color}
                visible={flowActive && activeShard === i}
              />
            );
          }
          return dots;
      ...
      <div
        style={{
          position: 'absolute',
          left: `${routerPos.fx * 100}%`,
          top: `${routerPos.fy * 100}%`,
      ...
      {shards.map((s, i) => (
        <React.Fragment key={`shard-${i}`}>
          <DbNode
            ...
            height={90}
            borderColor={s.color}
            springVal={shardSprings[i]}
            isActive={activeShard === i}
            frame={frame}
          />
...
const FailoverVariant: React.FC<Omit<DatabaseVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  const primary = { fx: 0.3, fy: 0.35 };
  const standby = { fx: 0.7, fy: 0.35 };
  const clientPos = { fx: 0.5, fy: 0.1 };

  // Phase: normal -> failure -> promotion
  const failureP = progressWindow(p, 0.35, 0.5);
  const promotionP = progressWindow(p, 0.55, 0.7);
  ...
  const trafficArrow = {
    x1: clientPos.fx * svgW,
    y1: clientPos.fy * svgH + 30,
    x2: target.fx * svgW,
    y2: target.fy * svgH - 50,
  };
    ...
    >
      ...
      >
        ...
        <FlowArrow
          {...trafficArrow}
          progress={arrowDrawP}
          color={standbyPromoted ? C.teal : C.saffron}
          frame={frame}
        ...
        {primaryDead && (
          <g opacity={0.6 + 0.4 * Math.sin(frame * 0.1)}>
            <line
              x1={primary.fx * svgW - 30}
              y1={primary.fy * svgH - 30}
              x2={primary.fx * svgW + 30}
              y2={primary.fy * svgH + 30}
              stroke={C.red}
              strokeWidth={4}
            />
            <line
      ...
      <DbNode
        ...
        y={`${primary.fy * 100}%`}
        label="PRIMARY"
        sublabel={primaryDead ? 'FAILED' : 'Active'}
        width={120}
        height={100}
        borderColor={primaryDead ? C.red : C.gold}
        springVal={nodeSpring}
        isActive={!primaryDead && flowActive}
        frame={frame}
        isDead={primaryDead}
      />
      ...
      <DbNode
        ...
        springVal={nodeSpring}
        isActive={standbyPromoted && flowActive}
        frame={frame}
      />
      ...
      {failureP > 0.3 && (
        <div
          style={{
            ...
            borderRadius: 10,
            padding: '10px 24px',
            opacity: interpolate(failureP, [0.3, 0.5], [0, 1], {
