...
const LookupVariant: React.FC<Omit<CacheVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  // Reveals
  const revealP = progressWindow(p, 0, 0.2);
  ...
  const arrowDrawP = progressWindow(p, 0.2, 0.4);
  const flowActive = p > 0.4;
  ...
  // Arrows
  const clientToCache = {
    x1: clientPos.fx * svgW + 50,
    y1: clientPos.fy * svgH,
    x2: cachePos.fx * svgW - 80,
    y2: cachePos.fy * svgH,
  };
  const cacheToDb = {
    x1: cachePos.fx * svgW + 80,
    y1: cachePos.fy * svgH,
    x2: dbPos.fx * svgW - 50,
    y2: dbPos.fy * svgH,
  };
  const cacheReturn = {
    x1: cachePos.fx * svgW - 80,
    y1: cachePos.fy * svgH + 20,
    x2: clientPos.fx * svgW + 50,
    y2: clientPos.fy * svgH + 20,
  };
  const dbReturn = {
    x1: dbPos.fx * svgW - 50,
    y1: dbPos.fy * svgH + 20,
    x2: cachePos.fx * svgW + 80,
    y2: cachePos.fy * svgH + 20,
  };
    ...
    >
      ...
      >
        ...
        <g opacity={arrowDrawP}>
          ...
          />
          <text
            x={(clientToCache.x1 + clientToCache.x2) / 2}
            y={clientToCache.y1 - 12}
            fill={C.saffron}
            fontSize={10}
            fontWeight={600}
            fontFamily="Inter, sans-serif"
        ...
        {flowActive && isHit && (
          <g opacity={0.4 + 0.3 * Math.sin(frame * 0.06)}>
            ...
            />
            <text
              x={(cacheReturn.x1 + cacheReturn.x2) / 2}
              y={cacheReturn.y1 + 16}
              fill={C.green}
              fontSize={10}
              fontWeight={600}
        ...
        {flowActive && (
          <FlowDot
            t={cyclePhase < 0.3 ? cyclePhase / 0.3 : 1}
            x1={clientToCache.x1}
            y1={clientToCache.y1}
            x2={clientToCache.x2}
            y2={clientToCache.y2}
            color={C.saffron}
            visible={cyclePhase < 0.3}
            size={6}
          />
        )}
      ...
      <div
        style={{
          position: 'absolute',
          left: `${cachePos.fx * 100}%`,
          top: `${(cachePos.fy - 0.15) * 100}%`,
          transform: `translate(-50%, -50%) scale(${nodeSpring})`,
          opacity: nodeSpring,
        }}
      >
      ...
      >
        ...
        {Array.from({ length: maxSlots }).map((_, i) => {
          ...
          return (
            <g key={`slot-${i}`} opacity={nodeSpring}>
              ...
              {entry && (
                <text
                  x={cacheX + cacheW / 2 + shakeX}
                  y={slotY + slotH / 2}
                  fill={isEvicting ? C.red : C.teal}
                  fontSize={12}
                  fontWeight={600}
                  fontFamily="Inter, sans-serif"
                  textAnchor="middle"
        ...
        {evicting && evictPhase < 0.5 && (
          <g opacity={evictPhase * 2}>
            <line
              x1={cacheX - 60}
              y1={cacheY + maxSlots * slotH}
              x2={cacheX + 8}
              y2={cacheY + maxSlots * slotH}
              stroke={C.green}
              strokeWidth={2}
              strokeLinecap="round"
            />
        ...
        <rect
          x={cacheX}
          y={cacheY + maxSlots * slotH + 30}
          width={cacheW * (filledSlots / maxSlots)}
          height={10}
          rx={5}
...
const LayersVariant: React.FC<Omit<CacheVizProps, 'variant'>> = ({ sync, frame }) => {
  ...
  const layers = [
    ...
    { label: 'Database', sublabel: '50-200ms', color: C.saffron, fy: 0.8, hitRate: 100 },
  ];
    ...
    >
      ...
      >
        ...
        {layers.map((layer, i) => {
          ...
          const isCatch =
            flowActive && i === catchLayer && cascadePhase > (catchLayer + 0.5) / layers.length;
            ...
            if (currentLayer <= targetLayer && layerProgress < 1) {
              ...
              return (
                <circle
                  ...
                  fill={C.saffron}
                  opacity={0.8}
                  style={{ filter: `drop-shadow(0 0 6px ${C.saffron})` }}
                />
              );
            }
            return null;
