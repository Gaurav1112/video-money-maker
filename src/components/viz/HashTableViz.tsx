...
export const HashTableViz: React.FC<HashTableVizProps> = ({ sync, frame, keywords, variant }) => {
  ...
  if (variant === 'collision')
    return <HashCollisionVariant sync={sync} frame={frame} keywords={keywords} />;
  if (variant === 'resize')
    return <HashResizeVariant sync={sync} frame={frame} keywords={keywords} />;
  if (variant === 'lookup')
    return <HashLookupVariant sync={sync} frame={frame} keywords={keywords} />;
  ...
  return (
    ...
    >
      ...
      {bucketSlots.map((i) => {
        ...
        const isHighlighted = (i === 0 && bucket0Highlighted) || (i === 2 && bucket2Highlighted);
      ...
      {loadP > 0 && (
        ...
        >
          ...
          >
            ...
            <span style={{ color: COLORS.gray, fontSize: 13 }}>3 keys / 5 buckets = 0.60</span>
          </div>
    ...
    >
      ...
      {bucketSlots.map((i) => {
        ...
        return (
          <BucketSlot
            ...
            index={i}
            opacity={slotP}
            translateX={interpolate(slotP, [0, 1], [-30, 0], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            })}
            highlighted={isHighlighted}
      ...
      {items.map((item, idx) => {
        ...
        const nodeSpring = spring({
          frame: Math.max(0, frame - Math.round(itemTriggers[idx] * fps * 10)),
          fps,
          config: { damping: 12, stiffness: 120, mass: 0.7 },
        });
        return (
      ...
      {p > 0.4 && (
        <div
          style={{
            ...
            justifyContent: 'center',
            gap: 20,
            opacity: interpolate(p, [0.4, 0.5], [0, 1], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            }),
          }}
        >
          ...
          >
            <span style={{ fontSize: 14, color: COLORS.saffron, fontWeight: 700 }}>
              Collisions:{' '}
              {Math.min(items.length - 1, Math.floor(progressInRange(p, 0.3, 0.7) * items.length))}
            </span>
          </div>
          <div
            style={{
          ...
          >
            <span style={{ fontSize: 14, color: COLORS.indigo, fontWeight: 700 }}>
              Lookup: O(
              {Math.min(items.length, Math.floor(progressInRange(p, 0.3, 0.7) * items.length) + 1)})
            </span>
          </div>
        </div>
      )}
      ...
      <div style={{ position: 'absolute', top: 50, left: 40, right: 40 }}>
        ...
        >
          <span>
            Load Factor:{' '}
            <span
              style={{ color: loadFactor > 0.75 ? COLORS.saffron : COLORS.gold, fontWeight: 700 }}
            >
        ...
        >
          <div
            style={{
              ...
              height: '100%',
              borderRadius: 6,
              background: loadFactor > 0.75 ? COLORS.saffron : COLORS.teal,
          ...
          {Array.from({ length: oldBuckets }).map((_, i) => (
            <div
              ...
              style={{
                ...
                justifyContent: 'center',
                fontSize: 14,
                color: i < filledCount ? COLORS.teal : COLORS.gray,
                fontFamily: 'JetBrains Mono, monospace',
              }}
            >
              {i < filledCount ? `k${i}` : '\u2014'}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  justifyContent: 'center',
                  fontSize: 12,
                  color: rehashed ? COLORS.gold : COLORS.gray,
                  fontFamily: 'JetBrains Mono, monospace',
                  transform: `scale(${rehashed ? 1 : 0.9})`,
                  transition: 'all 0.2s',
                }}
              >
                {rehashed ? `k${i}` : '\u2014'}
      ...
      {prePopulated.map((item) => {
        ...
        return (
          <HashNode
            key={item.key}
            label={`"${item.key}"`}
            x={160 + item.chainPos * 100}
            y={bucketY + 8}
            opacity={nodeP}
            scale={nodeP}
            color={nodeColor}
            isCollision={item.chainPos > 0}
          />
        );
      })}
      ...
      {traverseP > 0 && !isFound && (
        <div
          style={{
            ...
            borderRadius: 4,
            padding: '2px 8px',
            opacity: 0.5 + 0.5 * Math.sin(frame * 0.2),
          }}
        >
