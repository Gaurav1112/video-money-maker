...
const IntroSlide: React.FC<IntroSlideProps> = ({ topic = '', durationInFrames = 90, textHook }) => {
  ...
  if (textHook) {
    ...
    const logoBreathe =
      1 + interpolate(Math.sin(Math.max(0, frame - 20) * 0.06), [-1, 1], [0, 0.04]);

    // Hook text slam-in — more dramatic delay
    const textSpring = spring({
      frame: Math.max(0, frame - phase1End),
      fps,
    ...
    const textOpacity = interpolate(
      frame,
      [phase1End, phase1End + 6, phase3End, durationInFrames],
      [0, 1, 1, 0],
      {
        extrapolateLeft: 'clamp',
        extrapolateRight: 'clamp',
    ...
    const brandSpring = spring({
      frame: Math.max(0, frame - phase3Start),
      fps,
      ...
      <AbsoluteFill style={{ backgroundColor: '#0C0A15', overflow: 'hidden' }}>
        ...
        <div
          style={{
            ...
            opacity: interpolate(frame, [0, 5, 15, 20], [0, 0.8, 0.6, 0], {
              extrapolateLeft: 'clamp',
              extrapolateRight: 'clamp',
            }),
          ...
          >
            ...
            <img
              ...
              style={{
                ...
                height: 100,
                position: 'relative',
                filter: `drop-shadow(0 0 30px ${matchedCompany.color}88)`,
              }}
            />
          </div>
        )}
        ...
        >
          ...
          >
            ...
            >
              {matchedCompany
                ? `${matchedCompany.name.toUpperCase()} INTERVIEW`
                : 'INTERVIEW QUESTION'}
            </span>
          </div>
        </div>
  ...
  const codeRainOpacity = interpolate(
    frame,
    [0, 10, COUNTDOWN_END, COUNTDOWN_END + 20],
    [0, 1, 1, 0],
    {
      extrapolateLeft: 'clamp',
      extrapolateRight: 'clamp',
    ...
    return (
      <div
        ...
        style={{
          ...
          fontWeight: 500,
          color: COLORS.teal,
          opacity: drop.opacity * codeRainOpacity,
          whiteSpace: 'nowrap',
          textShadow: `0 0 8px ${COLORS.teal}40`,
        }}
      >
        {drop.keyword}
      </div>
      ...
      <div
        style={{
          ...
          opacity:
            frame < LOGO_START
              ? 0
              : interpolate(logoFrame, [0, 15], [0.6, 0.3], {
                  extrapolateLeft: 'clamp',
                  extrapolateRight: 'clamp',
                }),
          filter: 'blur(25px)',
      ...
      {topic && frame >= typewriterStart && (
        <div
          style={{
            ...
            letterSpacing: 1,
            textAlign: 'center',
            textShadow: `0 0 20px ${COLORS.teal}40`,
          }}
        >
