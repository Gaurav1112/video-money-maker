    ...
    <AbsoluteFill style={{ backgroundColor: '#0C0A15' }}>
      ...
      >
        ...
        {(() => {
          ...
          const splitAt = words.length <= 3 ? words.length : words.length <= 6 ? 2 : 3;
      ...
      >
        ...
        {totalSessions && totalSessions > 1 && (
          ...
          >
            ...
            {Math.round((sessionNumber / totalSessions) * 100)}% interview-ready
          </div>
        )}
        <div
          style={{
...
const VerticalTopicHeader: React.FC<{ topic: string; sessionNumber: number }> = ({
  topic,
  sessionNumber,
}) => (
  <div
...
const VerticalSceneContent: React.FC<{ scene: Scene; storyboard: Storyboard }> = ({
  scene,
  storyboard,
}) => {
      ...
      >
        ...
        {sceneBullets.map((bullet, i) => (
          ...
          >
            ...
            >
              {typeof bullet === 'string' && bullet.length > 80
                ? bullet.slice(0, 80) + '...'
                : bullet}
            </span>
          </div>
...
export const VerticalLong: React.FC<VerticalLongProps> = ({ storyboard }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#0C0A15', width: WIDTH, height: HEIGHT }}>
      ...
      {/* ── Outro ── */}
      <Sequence
        from={VERTICAL_INTRO_DURATION + contentFrames}
      ...
      {!isIntro && !isOutro && (
        <>
          <VerticalTopicHeader topic={storyboard.topic} sessionNumber={storyboard.sessionNumber} />
        ...
        <Sequence from={VERTICAL_INTRO_DURATION}>
          <Audio
            ...
            volume={(f) => {
              ...
              const fadeOut = interpolate(f, [contentFrames - 8, contentFrames], [1, 0], {
                extrapolateLeft: 'clamp',
                extrapolateRight: 'clamp',
              });
...
export function calculateVerticalLongMetadata({ props }: { props: Record<string, unknown> }) {
